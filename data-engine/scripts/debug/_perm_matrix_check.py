...
覆盖（此前只测过 列级query + 表级delete 两个点，20260804 用户要求全量夯实）：
...
注意：
- edit_data/delete_data 工具在进程内无人审上下文会被核武闸先拦（生产环境批准后才会
...
API_KEY = os.environ.get("MGMT_API_KEY") or os.environ.get("API_KEY") or ""
if not API_KEY:
    raise SystemExit("请先设置 MGMT_API_KEY 环境变量（管理端 X-API-Key）再运行本探针")
