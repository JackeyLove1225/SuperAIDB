...
（81 节点/47 决策/34 叶子锁定 + 三项结构校验）；本层锁定"元数据×树"交叉一致性：

1. 域文件组织：7 文件节点分布锁定（每域 <30 节点），跨文件无重复 id
2. 元数据×树相容：全组合枚举（7行为×16对象），树路由结果与工具 intent_tags
...
from core.tool_registry import _tools, find_tools
...
# 已知偏移豁免（树链尾兜底/语义等价的历史行为，非标注错误）。
# 断言对称：豁免工具若未来树改动后不再产生不相容路径，须同步移出本表。
_DRIFT_EXEMPTIONS = {
    "add_column": "增域链尾：非字段/外键/索引对象兜底加字段",
    "alter_precision": "改域链尾：未识别对象兜底精度修改",
    "import_template": "导入域链尾：非文件对象兜底模板导入",
    "save_template": "导出链尾兜底保存模板；改模板=重新保存",
...
def test_domain_files_layout():
    ...
    expected = {"_root.yml": 7, "query.yml": 17, "insert.yml": 14,
                "update.yml": 12, "delete.yml": 18, "file.yml": 7, "_shared.yml": 6}
