...
验证 safe_* helper 函数和 SecurityContract.validate_identifier
能正确拦截各类 SQL 注入攻击。
...
# 已实测可利用的注入 payload：字段合法但值含闭合引号 + 注释
...
# 函数形态布尔预言机/时间 DoS 黑名单（checks.validate_where 词边界锁）：
# 黑名单正则被改弱时本组断言即红
from core.checks import validate_where as _vw
check("where SLEEP 被拒", not _vw("id = SLEEP(5)"))
check("where BENCHMARK 被拒", not _vw("id = BENCHMARK(1000000,MD5(1))"))
check("where CASE-WHEN 被拒", not _vw("name = CASE WHEN 1=1 THEN 'x' ELSE 'y' END"))
check("where EXTRACTVALUE 被拒", not _vw("id = EXTRACTVALUE(1,2)"))
check("where 合法函数名近似放行", _vw("name = 'case文件'"))
# 标量函数形态内存 DoS（无黑名单关键字、纯函数调用穿透值域括号）：
check("where randomblob 内存 DoS 被拒", not _vw("name = randomblob(999999999)"))
check("where zeroblob 被拒", not _vw("id = zeroblob(1000000)"))
check("where 引号内括号字面量放行", _vw("name = 'foo(bar)'"))
check("where IN 列表放行", _vw("id IN (1,2,3)"))


# ── 场景 15：标识符正则统一标准（全项目收敛）──
...
# mysql_driver 复用同一标准（本地正则须有长度上限）
...
# ── 场景 16：MySQL SET 参数化 + _map_type 白名单 ──
...
# _map_type 拒绝注入/非法形态（VARCHAR 前缀原样透传即注入面）
...
# update() SET 子句参数化实测（假连接，无需真实 MySQL）
class _FakeCursor:
    def __init__(self):
        self.rowcount = 1
        self.executed = []
...
# ── 场景 17：sqlite_driver col_type 驱动层校验（纵深防御）──
...
expect_value_error_fn(
    "add_column 注入类型实测拦截", _memdrv.add_column,
...
# ── 场景 18：PDF 解析缓存 JSON 加固（pickle RCE 消除）──
print("\n=== 14.18 解析缓存篡改拒绝 ===")
import tempfile
...
try:
    ...
    _c.commit()
    # 故意保持一个活动连接不关闭，实测恢复不再受文件锁影响（Windows）
...
# ── 实测：ContractDriver 包真实 SqliteDriver ──
...
check("execute 实测 CREATE INDEX 成功", True)
_cdrv.execute("PRAGMA foreign_keys=OFF")
_fk = _cdrv_raw.conn.execute("PRAGMA foreign_keys").fetchone()
check("execute 实测 PRAGMA 已生效", _fk is not None and _fk[0] == 0, f"got {_fk!r}")
_cdrv.execute("PRAGMA foreign_keys=ON")
expect_security_error("execute 实测堆叠被拦", _cdrv.execute,
                      "INSERT INTO t21 (name) VALUES ('x'); DROP TABLE t21")
expect_security_error("execute 实测 VACUUM 被拦", _cdrv.execute, "VACUUM")
# 字面量内分号真实落库
_cdrv.execute("INSERT INTO t21 (name) VALUES ('a;b')")
_row = _cdrv_raw.conn.execute("SELECT name FROM t21 WHERE id=1").fetchone()
check("execute 实测字面量分号落库", _row is not None and _row[0] == "a;b",
      f"got {_row!r}")
_cdrv_raw.close()


# ── 场景 22：文件工具收容闸敏感文件黑名单（.env/.pem/id_rsa 不可读）──
# 活路径真身：agent/tools/_shared._guard_file_path（process_file/upload_file 的唯一
# 入口闸；图路径 file_tools 下线后黑名单由本闸继承，20260824）
print("\n=== 14.22 文件工具收容闸敏感文件黑名单 ===")
from agent.tools._shared import _guard_file_path as _gfp

_r = _gfp("data-engine/uploads/.env")
check("收容闸 .env 被拒", _r and "敏感文件不可访问" in _r, f"got {_r!r}")
_r = _gfp("data-engine/uploads/app.pem")
check("收容闸 .pem 被拒", _r and "敏感文件不可访问" in _r, f"got {_r!r}")
_r = _gfp("data-engine/uploads/id_rsa")
check("收容闸 id_rsa 被拒", _r and "敏感文件不可访问" in _r, f"got {_r!r}")
_r = _gfp("data-engine/db/.vault/master.key")
check("收容闸 master.key 被拒", _r and "敏感文件不可访问" in _r, f"got {_r!r}")
_r = _gfp("data-engine/db/saga_journal/saga_20260830_x1.json")
check("收容闸 saga journal 被拒", _r and "敏感文件不可访问" in _r, f"got {_r!r}")
_r = _gfp("data-engine/logs/app.log")
check("收容闸 .log 被拒", _r and "敏感文件不可访问" in _r, f"got {_r!r}")
_r = _gfp("config/anything.txt")
check("收容闸 config 禁区", _r and "配置/凭证目录" in _r, f"got {_r!r}")
_r = _gfp("C:/Windows/system32/drivers/etc/hosts")
check("收容闸工作区外越界", _r and "越界" in _r, f"got {_r!r}")
check("收容闸正常文本放行", _gfp("data-engine/uploads/note.txt") is None)
...
for _expr in ["地区编码 = '001'", "地区 = '武汉'", "name = '张三'", "name LIKE '%张%'"]:
    ...
    _b = resolve_field(_expr, "students")
...
try:
    ...
    check("带空格别名命中", resolve_field("total score > 90") == "score > 90")
    check("无空格别名变体命中", resolve_field("totalscore > 90") == "score > 90")
finally:
    _settings.INDUSTRY = _orig_ind23
    _shutil23.rmtree(_ns_dir, ignore_errors=True)

# 23.3 单表查询后选择集已创建（内存库隔离：monkeypatch data_ops.get_driver）
...
_origget_driver23 = _do23.get_driver
_do23.get_driver = lambda: _mem23
...
try:
    ...
    _do23.get_driver = _origget_driver23
...
_deny_ddl = {"default": "full", "users": {"bob": {"deny": ["ddl", "drop"]}}}
from core.permission import set_current_user as _set_user
...
_expect_denied("rekey 硬阻断（改钥后门）", "PRAGMA rekey='attacker_key'")
guard_write_sql("PRAGMA foreign_keys=ON")  # 合法 PRAGMA 不误伤
check("合法 PRAGMA 不误伤", True)

# schema 前缀归一（写护栏曾只捕 `main`，users 只读闸被
# UPDATE main.users 穿透实锤）——四形态前缀全部落回末段表名
_expect_denied("schema 前缀 UPDATE 穿透封堵", "UPDATE main.users SET role='admin' WHERE id=1")
_expect_denied("schema 前缀引号形态封堵", 'UPDATE "main"."users" SET role=\'admin\' WHERE id=1')
_expect_denied("schema 前缀方括号形态封堵", "UPDATE [main].[users] SET role='admin' WHERE id=1")
_expect_denied("schema 前缀 DELETE 封堵", "DELETE FROM main.users WHERE id=1")
_expect_denied("schema 前缀 INSERT 封堵",
               "INSERT INTO main.users (username,role) VALUES ('x','admin')")
_expect_denied("schema 前缀 CTE 删除封堵", "WITH x AS (SELECT 1) DELETE FROM main.users WHERE id=1")
# 变体（首轮修复只覆盖对称引号，这些曾穿透实锤）
_expect_denied("schema 前缀空白点号封堵", "UPDATE main . users SET role='admin' WHERE id=1")
_expect_denied("schema 前缀不对称引号左封堵", 'UPDATE "main".users SET role=\'admin\' WHERE id=1')
_expect_denied("schema 前缀不对称引号右封堵", 'UPDATE main."users" SET role=\'admin\' WHERE id=1')
_expect_denied("schema 前缀反引号封堵", "UPDATE `main`.`users` SET role='admin' WHERE id=1")
# schema 前缀形态下列级闸不得空转——必须用【非内置表+自定义列规则】
# 才能与表级闸区分（users 表级只读会掩盖列级是否生效）
_col_rules = {"default": "full", "datasources": {"primary": {"mode": "full",
    "tables": {"t_biz": {"columns": {"secret_col": {"deny": ["update", "insert"]}}}}}}}
with _patch("core.permission.policy.PermissionPolicy.get_instance",
            classmethod(lambda cls: PermissionPolicy.new_instance(_col_rules))):
    _expect_denied("schema 前缀 UPDATE 列级封堵（A1b 判别用例）",
                   "UPDATE main.t_biz SET secret_col='x' WHERE id=1")
    _expect_denied("schema 前缀空白 UPDATE 列级封堵（A1b 判别用例）",
                   "UPDATE main . t_biz SET secret_col='x' WHERE id=1")
    _expect_denied("schema 前缀 INSERT 列级封堵（A1b 判别用例）",
                   "INSERT INTO main.t_biz (name,secret_col) VALUES ('a','x')")
    guard_write_sql("UPDATE main.t_biz SET name='ok' WHERE id=1")  # 非禁列不误伤
# 合法 schema 前缀业务表不误伤
guard_write_sql("UPDATE main.t_biz SET v='x' WHERE id=1")
guard_write_sql("UPDATE main . t_biz SET v='x' WHERE id=1")
check("schema 前缀普通业务表不误伤（含空白点号）", True)

# PRAGMA 白名单（可写 PRAGMA 曾实跑改写库头）——可写项 fail-closed
_expect_denied("PRAGMA user_version 可写项拦截", "PRAGMA user_version=999")
_expect_denied("PRAGMA schema_version 可写项拦截", "PRAGMA schema_version=7")
_expect_denied("PRAGMA journal_mode 持久变更拦截", "PRAGMA journal_mode=DELETE")
guard_write_sql("PRAGMA table_info(users)")   # 只读查询不误伤
check("只读/会话 PRAGMA 不误伤", True)
