...
def _create_test_excel(path: str):
    ...
    ws3 = wb.create_sheet("空表")  # 空表，验证不报错
    wb.save(path)
...
def test_process_file_excel_unit_display():
    ...
    captured = {}
    def mock_run(filepath, industry, page_start, page_limit, batch_size, overwrite, only_tables=None):
    ...
    with patch('config.settings.settings') as mock_settings, \
         ...
         patch('pipeline.runner._check_file_size') as mock_size, \
         patch('core.data_ops._get_driver') as mock_drv, \
         patch('industries.base.discover_industries') as mock_disc, \
...
def _run_batch_process(ai, pages, page_limit, overlap, output_dir=None):
    ...
    with patch("core.data_ops._get_driver", return_value=_OverlapFakeDriver()):
...
def test_llm_budget_guard():
    """P1-6：LLM 预算护栏——超限中止、已处理批次保留、units 定位续入点"""
    from unittest.mock import patch
    from pipeline.runner import batch_process
    ...
    print("OK - P1-6 LLM 预算护栏：超限中止+已处理保留+续入定位")
...
def test_extract_completeness_retry():
    ...
    print("OK - P1-7 提取完整性校验：低提取自动重提取多者、充足不重提")
