...
def _unregister(name):
    from core.tool_registry import unregister_tool
...
def test_schema_manager_force_chain():
    ...
    # ── modify_column：TEXT → FLOAT 危险转换（TypeContract 矩阵：跨族 danger 需 force；
    # 预检与执行闸同源——旧用例的 FLOAT→TEXT 在契约矩阵是 safe 无损，
    # 曾是被并行第二套规则（checks.validate_type_change）判错的漂移形态）──
    ...
    with ps[0], ps[1], ps[2], ps[3], ps[4]:
        r3 = sm.modify_column("t", "c", "FLOAT")
        assert r3.get("need_force") is True, r3  # TEXT→FLOAT 跨族必弹 force 卡
        drv2.modify_column.assert_not_called()
        r4 = sm.modify_column("t", "c", "FLOAT", force=True)
        assert r4.get("ok") is True, r4
        drv2.modify_column.assert_called_once_with("t", "c", "FLOAT", force=True)
...
def test_mcp_channel_force_bridge():
    """7. MCP 通道 force 桥（20260824）：need_force 不再是永久死路——
    登记挂起（pending_approval 回执，token 不回传）→ 管理端结算带 force 执行；
    同一风险重复触发（AI 带 force=true 重试撞前置闸）查重不重复登记。"""
    ...
    def handler(force=False):
        ...
        if not force:
            return ToolResult.fail("危险变更需确认", code="VALIDATION",
                                   reason="need_force")
        return ToolResult.ok("已带 force 执行")

    _register_tmp_tool("tmp_force_mcp", handler, ["force"])
    ctx = get_context()
    old_ch = ctx.get_channel()
    ctx.set_channel("mcp")
    try:
        r = execute_tool("tmp_force_mcp")
        assert r.data.get("reason") == "pending_approval", \
            f"MCP 通道 need_force 应走挂起表而非死路拒绝: {r.data}"
        assert calls == [False], "未批准前不得执行"
        pend = [p for p in list_pending() if p["name"] == "tmp_force_mcp"]
        assert len(pend) == 1, f"应登记一项挂起: {pend}"
        # 查重：同风险再触发（前置闸路径）不重复登记
        r2 = execute_tool("tmp_force_mcp", force=True)
        assert r2.data.get("reason") == "pending_approval", r2.data
        assert "已在审批队列" in r2.text or "待批准" in r2.text, r2.text[:120]
        pend2 = [p for p in list_pending() if p["name"] == "tmp_force_mcp"]
        assert len(pend2) == 1, f"同一风险不得重复登记: {len(pend2)}"
        token = pend2[0]["token"]
        # 管理端结算（与 approvals.settle 同机制）：批量预批准 + kwargs 自带 force
        op = pop_pending(token)
        assert op and op["kwargs"].get("force") is True, op
        ctx.set_nuke_batch(tables={"*"}, ops={"tmp_force_mcp"})
        try:
            r3 = execute_tool("tmp_force_mcp", **op["kwargs"])
        finally:
            ctx.clear_nuke_batch()
        assert r3.data.get("ok"), f"结算后应带 force 执行: {r3.data}"
        assert calls == [False, True], calls
    finally:
        ctx.set_channel(old_ch)
        _unregister("tmp_force_mcp")
        for p in list_pending():
            if p["name"] == "tmp_force_mcp":
                pop_pending(p["token"])
    print("OK - MCP force 桥：挂起登记/查重/结算带 force 执行全链路")
