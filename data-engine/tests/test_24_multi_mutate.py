...
def test_multi_mutate_mcp_bridge():
    """多表合并闸 MCP 桥（20260824）：MCP 通道不再是"当前环境不支持
    确认"死路——登记单项挂起（pending_approval，token 不回传）→ 管理端结算
    重放原指令（批量预批准命中免检）→ 真实删库。"""
    import core.data_ops as ops
    ...
    ctx = get_context()
    old_ch = ctx.get_channel()
    ctx.set_channel("mcp")
    try:
        with patch.object(ops, "_load_table_schema", _fake_schema), \
             patch.object(ops, "_extract_mutation_ops", lambda _i: ops_parsed):
            r = ops.mutate_natural("删除父表 id=1 与子表 pid=1 的记录")
        assert r.data.get("reason") == "pending_approval", \
            f"MCP 通道多表应走挂起表而非死路拒绝: {r.data}"
        pend = [p for p in list_pending() if p["name"] == "__multi_mutate__"]
        assert len(pend) == 1, f"应登记一项多表挂起: {pend}"
        drv = ops.get_driver()
        assert drv.query("SELECT COUNT(*) c FROM t_parent")[0]["c"] == 1, "未批准不得删"
        # 管理端结算（与 approvals.settle 的 __multi_mutate__ 特判同机制：
        # 冻结 ops 重放，不重跑 LLM 提取）
        op = pop_pending(pend[0]["token"])
        assert op["kwargs"].get("_ops_frozen"), "登记应冻结结构化 ops"
        assert op["kwargs"].get("_expect_ids"), "登记应冻结候选 id 集"
        ctx.set_nuke_batch(tables=set(op["kwargs"]["_tables"]),
                           ops=set(op["kwargs"]["_ops"]))
        try:
            with patch.object(ops, "_load_table_schema", _fake_schema):
                r2 = ops.mutate_natural(op["kwargs"]["instruction"],
                                        ops_override=op["kwargs"]["_ops_frozen"],
                                        expect_ids=op["kwargs"]["_expect_ids"])
        finally:
            ctx.clear_nuke_batch()
        assert r2.data.get("ok") is True, r2.data
        assert drv.query("SELECT COUNT(*) c FROM t_parent")[0]["c"] == 0
        assert drv.query("SELECT COUNT(*) c FROM t_child")[0]["c"] == 0

        # 漂移拒绝（TOCTOU）：结算后两表已空——各放回一条候选 →
        # 登记挂起 → 挂起后再各灌一条（候选 1→2 双漂移）→ 结算拒绝
        drv.execute("INSERT INTO t_parent (id) VALUES (1)")
        drv.execute("INSERT INTO t_child (pid) VALUES (1)")
        drv.commit()
        with patch.object(ops, "_load_table_schema", _fake_schema), \
             patch.object(ops, "_extract_mutation_ops", lambda _i: ops_parsed):
            ops.mutate_natural("删除父表 id=1 与子表 pid=1 的记录")
        pend2 = [p for p in list_pending() if p["name"] == "__multi_mutate__"]
        assert len(pend2) == 1
        # 挂起后往两表各灌一条匹配行（两表候选 1→2 都漂移——
        # 只漂移子表会让父表落单走单表路径，测不到多表漂移语义）
        drv.execute("INSERT INTO t_parent (id) VALUES (2)")
        drv.execute("INSERT INTO t_child (pid) VALUES (1)")
        drv.commit()
        op2 = pop_pending(pend2[0]["token"])
        ctx.set_nuke_batch(tables=set(op2["kwargs"]["_tables"]),
                           ops=set(op2["kwargs"]["_ops"]))
        try:
            with patch.object(ops, "_load_table_schema", _fake_schema):
                r3 = ops.mutate_natural(op2["kwargs"]["instruction"],
                                        ops_override=op2["kwargs"]["_ops_frozen"],
                                        expect_ids=op2["kwargs"]["_expect_ids"])
        finally:
            ctx.clear_nuke_batch()
        assert "漂移" in str(r3) and r3.data.get("reason") == "candidate_drift", \
            f"候选漂移应拒绝执行: {str(r3)[:120]}"
        assert drv.query("SELECT COUNT(*) c FROM t_child")[0]["c"] == 2, \
            "漂移拒绝后数据不得被删"
    finally:
        ctx.set_channel(old_ch)
        _teardown_scratch_db()
        for p in list_pending():
            if p["name"] == "__multi_mutate__":
                pop_pending(p["token"])
    print("OK - 多表 MCP 桥：挂起登记→结算重放→真实删库（不再是死路）")
