...
def _make_service(fk_rows, table_meta):
    ...
    patch("core.steward.Steward.get_driver", return_value=drv).start()
...
def test_merge_preserves_business_meta():
    """_merge_table_updates：前端模型不带/空带 business_name/description 时
    按字段名从旧 schema 继承（防"改个类型把全表业务名抹掉"的元数据丢失回归）"""
    from core.graph.schema_graph_service import SchemaGraphService as S
    old = {"name": "t", "business_name": "表", "columns": [
        {"name": "id", "type": "INTEGER", "business_name": "主键"},
        {"name": "rate", "type": "FLOAT", "business_name": "单价", "description": "d"},
    ]}
    updates = {"columns": [
        {"name": "id", "type": "INTEGER"},
        {"name": "rate", "type": "INTEGER", "business_name": ""},
        {"name": "newc", "type": "TEXT", "business_name": "新列"},
    ]}
    merged = S._merge_table_updates(old, updates, "t")
    cols = {c["name"]: c for c in merged["columns"]}
    assert cols["id"]["business_name"] == "主键", cols["id"]
    assert cols["rate"]["business_name"] == "单价" and cols["rate"]["description"] == "d"
    assert cols["rate"]["type"] == "INTEGER"  # 结构以新值为准
    assert cols["newc"]["business_name"] == "新列"
    print("OK - 合并更新：业务元数据按名继承、结构以新值为准")
...
if __name__ == "__main__":
    ...
    test_merge_preserves_business_meta()
    print("\n层 31 全绿")
