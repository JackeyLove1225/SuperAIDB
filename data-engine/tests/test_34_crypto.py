...
def test_crypto_end_to_end():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # INSERT+commit 都会写主文件刷新 mtime，与是否迁移无关，会误报）
...
def test_polluted_key_rejected_and_chmod():
    """密钥字符集校验（单一收口锁）+ 密钥文件 0600 权限（POSIX 下断言，
    Windows chmod 语义尽力而为则跳过权限位断言）"""
    import os, stat, sys
    from core.crypto import key_manager as km
    from core.crypto.key_manager import _validated_key
    # 1) 字符集校验：污染形态（单引号/分号/空白/中文）一律 RuntimeError
    for bad in ("abc'; DROP TABLE users; --", "a b", "密钥", "a;b"):
        try:
            _validated_key(bad)
            raise AssertionError(f"污染密钥未被拒绝: {bad!r}")
        except RuntimeError as e:
            assert "非法字符" in str(e)
    assert _validated_key("abcXYZ_09-ok") == "abcXYZ_09-ok"
    # 2) 隔离模式密钥文件 0600（生成与存量读取两路径同标准）
    import tempfile
    from pathlib import Path
    with tempfile.TemporaryDirectory() as tmp:
        os.environ["SUPERAIDB_VAULT_DIR"] = tmp
        flag = km._ISOLATED_FLAG
        created = False
        try:
            if not flag.exists():
                flag.parent.mkdir(parents=True, exist_ok=True)
                ...
                created = True
            key = km._get_key_file()
            kf = Path(tmp) / "master.key"
            assert kf.exists() and km._get_key_file() == key
            ...
            if created:
                flag.unlink(missing_ok=True)
            os.environ.pop("SUPERAIDB_VAULT_DIR", None)
    print("OK - 密钥字符集校验收口 + 密钥文件 0600（生成/存量同标准）")
...
if __name__ == "__main__":
    ...
    test_polluted_key_rejected_and_chmod()
