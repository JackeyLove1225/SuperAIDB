...
def extract_intermediate(text: str, ai, schemas: list = None,
                         focus_fields: list = None) -> dict:
    ...
    prompt = _UNIFIED_PROMPT.format(text=text[:60000],
                                    targets=_targets_hint(schemas or [])) + focus_hint
    last_err = ""
...
def _map_kv_block(kv: dict, field_idx: list, rules: dict,
                  out_tables: dict, unmapped: list) -> None:
    """kv：逐键映射，按命中数选最佳表（就地填充 out_tables/unmapped）"""
...
def _resolve_best_candidate(headers: list, direct: list, field_idx: list):
    """逐表上下文消解：候选表内逐列解歧（全局歧义的列在表内唯一即可配），
    以"解出的列占比"选表——严格 >0.5 才映射，杜绝恰半混合表蒙对；
    同表两列同字段（colspan 不规则）该表直接判 0 分

    返回 (score, exact, table, field_of) 最优候选；无候选返回 None
    """
    cand_tables = []
    ...
    return cand_tables[0] if cand_tables else None


def _assemble_table_rows(best, headers: list, rows: list, direct: list,
                         schemas: list, out_tables: dict, unmapped: list) -> None:
    """按最优候选装配表行（数值列 NFKC 归一 + 链接列保留），落选列进 unmapped"""
    ...
    for r in rows:
        ...
        for _nf in num_fields:
            if _nf in row and isinstance(row[_nf], str):
                row[_nf] = unicodedata.normalize("NFKC", row[_nf]).strip()
...
def _map_source_table(it: dict, field_idx: list, rules: dict, schemas: list,
                      out_tables: dict, unmapped: list) -> None:
    """单个源表：转置检测 → 候选消解 → 行装配；覆盖不足则整张进 unmapped"""
    ...
    for i, h in enumerate(headers):
        ...
        if f:
            direct.append((i, t, f))
    best = _resolve_best_candidate(headers, direct, field_idx)

    if best and best[0] > 0.5:
        _assemble_table_rows(best, headers, rows, direct, schemas, out_tables, unmapped)
    else:
...
def map_to_schemas(intermediate: dict, schemas: list, aliases: dict = None,
    ...
    if kv:
        _map_kv_block(kv, field_idx, rules, out_tables, unmapped)
    # tables：按表头覆盖度映射
    for it in intermediate.get("tables") or []:
        _map_source_table(it, field_idx, rules, schemas, out_tables, unmapped)
...
def batch_process_unified(file_path, config, stream, ai, industry: str,
    ...
    from pipeline.routing import route_tables
    ...
    while start < total:
        ...
        if route:
            ...
            routed, _ = route_tables(config, sample, ai)
            batch_cfg = dataclasses.replace(config, tables=[t for t in config.tables if t.get("name") in routed])
        ...
        inter = extract_intermediate(prompt_text, ai, schemas=batch_cfg.tables,
                                     focus_fields=getattr(batch_cfg, 'focus_fields', None))
        if inter.get("_error"):
...
def _apply_confirmation_intent(text: str, items: list, industry: str,
                               valid: set, saved: list, ignored: list) -> tuple:
    """解析确认意图并应用（存档映射/忽略规则，就地填充 saved/ignored）

    返回 (是否命中映射指令, 直接回复或 None, 剩余待确认项)：
    - (False, "", items)：不是映射确认指令
    - (True, 回复文本, remaining)：显式规则校验失败，直接回复（不动挂起）
    - (True, None, remaining)：已应用，由调用方继续补录与装配回复
    """
    remaining: list = list(items)

    def _confirm_item(p):
        if p.get("guess_field") and (p["guess_table"], p["guess_field"]) in valid:
            save_mapping_rule(industry, p["key"], p["guess_table"], p["guess_field"])
            saved.append(p)
            return True
        return False

    def _ignore_item(p):
        save_ignore_rule(industry, p["key"])
        ignored.append(p)
    ...
    if m:
        ...
        if (table, field) not in valid:
            return True, (f"映射校验失败：{table}.{field} 不存在于当前行业。"
                          f"请检查表名/字段名（区分大小写），或回“忽略”。"), remaining
        save_mapping_rule(industry, key, table, field)
...
def _reingest_saved(saved: list, cfg, rules: dict, aliases: dict) -> list:
    """免 AI 补录（规则即代码，白盒复算）：确认项保留的源数据直接入库

    返回补录结果行（每表一条：+N 条/冲突跳过/失败原因；写入异常也如实记录）
    """
    ingested_lines = []
    ...
    return ingested_lines


def _confirmation_reply(saved: list, ignored: list, ingested_lines: list,
                        remaining: list) -> str:
    """装配确认回复：已记映射/已忽略/补录结果/剩余待确认"""
    reply_parts = []
    if saved:
...
def handle_mapping_confirmation(user_input: str, pending: dict) -> tuple:
    ...
    saved: list = []
    ignored: list = []
    handled, direct_reply, remaining = _apply_confirmation_intent(
        text, items, settings.INDUSTRY, valid, saved, ignored)
    if not handled:
        return False, ""
    ...
    ingested_lines = _reingest_saved(saved, cfg, rules, aliases)
    ...
    return True, _confirmation_reply(saved, ignored, ingested_lines, remaining)


def _norm_code_value(v) -> str:
    """兼容别名——唯一实现已迁 pipeline.common.norm_code_value（跨阶段共享）"""
    from pipeline.common import norm_code_value
    return norm_code_value(v)
