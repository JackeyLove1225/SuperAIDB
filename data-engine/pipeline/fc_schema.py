...
def _build_fc_schema(config, drv=None):
    ...
    for t in config.tables:
        ...
        # 字段聚焦（用户指令指定提取字段子集）：按字段名/中文业务名匹配裁剪。
        # 保护面不受裁剪：唯一业务键（冲突检测/关联锚点）与外键列——
        # 业务编码丢了，明细关联与去重就断链，关联完整性优先
        focus = {f.strip().lower() for f in (getattr(config, "focus_fields", None) or []) if f.strip()}
        # 保护面同源（唯一实现 protected_keys_for_table——唯一业务键/主表编码/
        # 虚拟基础表编码/明细虚拟主表编码全在内，删了即断链）
        _protected = protected_keys_for_table(config, tname)
        for c in t.get("columns", []):
            cname = c["name"]
            if cname == "id":
                continue
            if focus and cname not in _protected \
                    and cname.lower() not in focus \
                    and (c.get("business_name") or "").lower() not in focus:
                continue  # 非聚焦字段不出现在提取 schema（AI 不会提取它们）
...
def protected_keys_for_table(config, tname: str) -> set:
    """字段聚焦保护面（唯一实现——fc_schema 裁剪与 runner 后处理同源）：
    本表全部声明列名 ∪ 主表业务编码（含无唯一索引回退识别）∪
    主表虚拟基础表编码（ref_code）∪ 明细表虚拟主表编码（关联键）。
    这些键在聚焦裁剪下永远保留——丢一个，明细断链/外键静默丢/去重失效。"""
    # 同模块函数直引（不自 import）
    main_table = _find_main_table(config)
    code_field = _find_code_field(config, main_table)
    keys = {"id"}
    ext = _get_extraction_tables(config)
    for t in config.tables:
        if t["name"] != tname:
            continue
        # 唯一业务键（冲突检测/去重锚点）
    ...
    if tname == main_table:
        if code_field:
            keys.add(code_field)
        for _fk_col, _rt, ref_code in _find_main_fks_to_base(config, main_table, ext):
            keys.add(ref_code)
    elif code_field:
        keys.add(code_field)  # 明细表虚拟主表编码（关联键，丢失即断链静默丢）
    return keys


def assert_focus_matched(config, drv=None) -> None:
    """字段聚焦零命中如实报（唯一实现——runner 入口与 fc 提取层同源，
    口径不漂移）：focus 字段一个都不命中时 raise ValueError（附可用字段
    清单），绝不静默按全表提取。"""
    focus = [f for f in (getattr(config, "focus_fields", None) or []) if f.strip()]
    if not focus:
        return
    fs = {f.lower() for f in focus}
    matched, avail = 0, []
    ext = _get_extraction_tables(config, drv)
    for t in config.tables:
        if t["name"] not in ext:
            continue  # 基础表不从文档提取，其字段命中不算有效聚焦
        for c in t.get("columns", []):
            if c["name"] == "id":
                continue
            avail.append(c.get("business_name") or c["name"])
            if c["name"].lower() in fs or (c.get("business_name") or "").lower() in fs:
                matched += 1
    if matched == 0:
        raise ValueError(
            f"指定字段未命中（{focus}）。当前行业可用字段：{'、'.join(avail[:30])}")
