...
def batch_process(file_path, config, stream, ai, page_limit=TIER2_BATCH_UNITS,
    ...
    未声明唯一约束的行业配置下兜底不触发。"""
    drv, system_prompt, functions, main_table, skip_if_zero = _prepare_extraction(config)
    ...
    while start < total:
        # 收集 page_limit 页
        chunk = _next_chunk(stream, page_limit)
        if not chunk:
            break

        # 批次级语义路由（混合域文件：每个批次按自身内容选表，证据闭环）
        batch_functions, batch_meta = _route_batch_schema(config, drv, ai, chunk, route, functions)
        if batch_functions is None:
            # 路由无相关表：本批跳过提取
            batch_num += 1
        ...
        prompt_text, ai_prompt = _build_ai_prompt(chunk, prev_tail, overlap)

        _dump_batch_raw(output_dir, batch_num, prompt_text)

        batch_num += 1
        logger.info("  第 %d 批: %d 页（overlap 上下文 %d 页）", batch_num, len(chunk),
                    len(prev_tail) if overlap > 0 else 0)

        # LLM 预算护栏：超预算即中止，已入库批次保留，剩余可续入
        if _budget_exceeded(budget):
            return

        # Function Calling 提取 + 提取完整性校验
        fn_args = _fc_extract(ai, batch_functions, ai_prompt, system_prompt,
                              max_tokens, chunk, budget)

        # 转成 pipeline 内部格式 {"tables": [{"name": ..., "rows": [...]}]}
        _main = batch_meta["main_table"] if batch_meta else main_table
        tables_out = _build_tables_result(fn_args, config, skip_if_zero, _main)
...
def _next_chunk(stream, page_limit):
    """从文本流收集至多 page_limit 个非空文本单元作为一批；流尽时返回空列表。"""
    chunk = []
...
def _dump_batch_raw(output_dir, batch_num, prompt_text):
...
def _budget_exceeded(budget):
    """LLM 预算护栏：超预算时置 stopped=True 并告警、返回 True（调用方中止）；否则 False。"""
...
def _prepare_extraction(config):
    """提取前置准备：从配置读取提取规则、构建 FC schema 与 system prompt。

    返回 (drv, system_prompt, functions, main_table, skip_if_zero)。
    所有表名/字段名/业务规则从配置读取，无硬编码。"""
    ...
    # 字段聚焦零命中如实报（第二关纵深——唯一实现 fc_schema.assert_focus_matched，
    # 与 runner 入口同版；不静默按全表提取，那是"答非所问"面）
    _focus = [f for f in (getattr(config, "focus_fields", None) or []) if f.strip()]
    if _focus:
        from pipeline.fc_schema import assert_focus_matched
        assert_focus_matched(config, drv)
    functions = [{
        "type": "function",
        "function": {
            "name": "output_data",
    ...
    if _focus:
        system_prompt += (
            "\n【字段聚焦】本次只提取以下字段：" + "、".join(_focus) + "。"
            "其余字段一律忽略，不要提取、不要猜测补全。\n")
    ...
    return drv, system_prompt, functions, main_table, skip_if_zero


def _route_batch_schema(config, drv, ai, chunk, route, default_functions):
    """批次级语义路由：按本批内容选表并重建批次级提取 schema。

    返回 (batch_functions, batch_meta)；route=False 时用全行业 schema（meta 为 None），
    路由无相关表时返回 (None, None)，表示本批跳过提取。"""
    if not route:
        return default_functions, None
    sample = chunk[0][:1500] if chunk else ""
    from core.llm_usage import set_role as _usage_role
    from pipeline.routing import route_tables  # 调用时局部导入：提取层消费路由服务，不形成模块级反向边
    with _usage_role("extract_file"):
        routed, _r_reason = route_tables(config, sample, ai)
    ...
    return batch_functions, batch_meta


def _build_ai_prompt(chunk, prev_tail, overlap):
    """拼接批次 prompt，返回 (prompt_text, ai_prompt)。

    prompt_text 仅当前批次页面（供原始文本落盘）；ai_prompt 在带 overlap 上下文时
    追加"请勿提取"标注的上下文段。"""
    prompt_text = "\n\n---\n\n".join(chunk)
    ...
    return prompt_text, ai_prompt


def _fc_extract(ai, batch_functions, ai_prompt, system_prompt, max_tokens, chunk, budget):
    """Function Calling 提取（AI 直接输出结构化数据）+ 完整性校验重提，返回 fn_args。

    LLM 调用计数在 budget 上原地累计；重提受预算护栏约束。"""
    # Function Calling — AI 直接输出结构化数据
    from core.llm_usage import set_role as _usage_role
    with _usage_role("extract_file"):
        _, fn_args = ai.call_function(
...
def _build_tables_result(fn_args, config, skip_if_zero, main_table):
    """FC 输出 → pipeline 内部格式 {"tables": [{"name": ..., "rows": [...]}]}。

    附零值过滤、逐表调试日志与主明细比例合理性检查。"""
    tables_out = []
    for t in config.tables:
        tname = t["name"]
        if tname in fn_args:
    ...
    # 主明细比例合理性检查：主表 0 行但明细 >0 行通常意味着主表漏提
    _main_rows = next((len(td["rows"]) for td in tables_out if td["name"] == main_table), 0)
    _detail_rows = sum(len(td["rows"]) for td in tables_out if td["name"] != main_table)
    if _main_rows == 0 and _detail_rows > 0:
        logger.warning("  提取比例异常: 主表 %s 0 行但明细共 %d 行（主表可能漏提，明细将无法关联）",
                       main_table, _detail_rows)
    return tables_out
