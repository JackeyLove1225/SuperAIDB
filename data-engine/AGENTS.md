...
- **MCP server（stdio，无端口）**：能力面——结构化指令契约（20260905 起）：MCP 面仅 5 个
  数据工具（execute_structured 结构化契约 + 人审机制三件套 + 只读辅助），一切数据操作
  由上层 AI 翻译成结构化契约（behavior+object+args，枚举真源=决策树）经树路由到达具体
  工具（仓内 39 个原子工具，按名直调在调用点封死，自然语言链 execute_instruction 保留
  服务 Web/测试不上 MCP 面）；对话编排由上层 AI 客户端承载（本仓无进程内聊天循环）
