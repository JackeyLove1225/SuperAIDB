...
def _require_admin(request: "Request | None") -> None:
    ...
    不校验角色——系统级/配置级写端点此前普通 user 登录即可调用。
...
def get_dashboard():
    ...
    # 前端状态（TCP 直连 + settings 端口——urllib 读系统代理会误判（07:08 事故），
    ...
    # Schema 三层漂移摘要（P1-8）：漂移存在时 dashboard 显示告警
...
def stop_all_services(request: Request = None):
    ...
    """
    _require_admin(request)
    pid_file = Path(_project_root) / ".backend_pids"
    ...
    def _do_stop():
        ...
        if pid_file.exists():
            try:
                ...
                if launcher_pid:
                    pids.append(launcher_pid)
            except Exception:
                pass

        # 杀掉端口占用的进程（包括子进程树）——
        # 身份校验收口：只杀本项目进程（_is_project_process 与 launcher 同款），
        # 端口取自 settings（幽灵端口 2024/7687/7474 已清除）——
        # 客户机器上 3000 被别的 dev server 占用时不再误杀（评审三轮运维复核）
        from agent.management.launcher import _is_project_process
        from config.settings import settings as _st
        for port in [_st.MGMT_PORT, _st.FRONTEND_PORT]:
            try:
        ...
        # 杀掉 PID 文件中的进程（同样过身份校验；_is_project_process 吃 int PID——
        # 曾误传 psutil.Process 对象，TypeError 被吞、整条 PID 通道静默失效）
        for pid in pids:
            try:
                proc = psutil.Process(pid)
                if not _is_project_process(pid):
                    continue
                for child in proc.children(recursive=True):
                    try:
                        child.kill()
