...
def insert_table_data(table_name: str, body: dict, request: Request):
    """新增数据行——经 DataCrudContract 契约校验

    Body: {"rows": [{...}, ...], "overwrite": false, "operator_password": str}
    - rows: 要插入的行列表（每行为字段名→值的 dict）
    - overwrite: 是否覆盖冲突行（默认 false）
    - operator_password: 写操作密码（写皆密码，查询才免签）
...
def _safe_rollback(drv):
    try:
        ...
        pass  # 回滚失败——连接已坏，句柄由 OS 回收


@router.put("/api/database/table/{table_name}/data")
def update_table_data(table_name: str, body: dict, request: Request):
    """更新数据行——经 DataCrudContract 契约校验

    Body: {"set": "name='张三', age=30", "where": "id=1", "operator_password": str}
...
def delete_table_data_by_pk(table_name: str, body: dict, request: Request):
    """按主键删除数据行——参数化绑定，前端不再手拼 SQL WHERE。

    body: {"pk_column": str, "pk_value": Any, "operator_password": str（删除需操作密码）}
    """
    from agent.management.deps import require_operator_password
    require_operator_password(request, body)
...
def update_table_data_by_pk(table_name: str, body: dict, request: Request):
    """按主键更新数据行——服务端字面值安全拼装（JSON 类型驱动），
    前端不再手拼 SQL SET/WHERE（对称 delete-by-pk；"拼装渗进展示层"的收口）。

    body: {"pk_column": str, "pk_value": Any, "values": {列名: 值, ...},
           "operator_password": str（写皆密码，查询才免签）}
...
def delete_table_data(table_name: str, where: str = "", body: dict = None, request: Request = None):
    """删除数据行——经 DataCrudContract 契约校验 + 操作密码（删除即不可逆）
