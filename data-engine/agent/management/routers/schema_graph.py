...
def schema_graph_create_table(body: dict):
...
def schema_graph_update_table(table_name: str, body: dict):
...
def schema_graph_delete_table(table_name: str, drop_real_table: bool = True):
    """删除表——同步四层
...
def schema_graph_create_relationship(body: dict):
...
def schema_graph_delete_relationship(body: dict):
    """删除外键关系——同步三层

    Body: {table_name, column_name}
    """
    table_name = body.get("table_name", "")
    column_name = body.get("column_name", "")
...
def schema_graph_verify():
    """三层对账端点（P1-8）：YAML ↔ MetaDB ↔ Ladybug 图库漂移检测。
...
def schema_graph_check_templates(col_type: str = Query("", alias="type")):
    ...
    """
    from core.graph.check_templates import get_templates_by_type, get_all_templates_flat, normalize_type
    try:
        if col_type:
