...
全部经 Bearer 认证保护（server 统一中间件；X-API-Key 系统通道已于
20260903 废除）。保存即生效（ConfigHub mtime 热生效），不需要重启任何进程。
"""
...
def _validate_rules(rules: dict) -> None:
    ...
    # 用户级规则（users.<用户名>，与角色同构，级联其后）
    for uname, urules in (rules.get("users") or {}).items():
        if not isinstance(urules, dict):
            raise HTTPException(status_code=400, detail=f"用户规则必须是对象: {uname}")
        if urules.get("allow") is not None and urules.get("deny") is not None:
            raise HTTPException(status_code=400, detail=f"用户 {uname} 的 allow/deny 不能同时配置")
        _validate_op_list(f"用户 {uname} 的 allow", urules.get("allow"))
        _validate_op_list(f"用户 {uname} 的 deny", urules.get("deny"))
        for tname, trules in (urules.get("tables") or {}).items():
            _validate_scope(f"users.{uname}.tables.{tname}", trules)
            for cname, crules in (trules.get("columns") or {}).items():
                if not isinstance(crules, dict):
                    raise HTTPException(status_code=400, detail=f"列规则必须是对象: {cname}")
                _validate_op_list(f"用户 {uname} 列 {cname} 的 deny", crules.get("deny"))
...
def _spelling_warnings(rules: dict) -> list:
    """拼写交叉核对：datasources/roles 键与注册表大小写不敏感
    比对——运行时查表已规范形收口（不再静默 fail-open），此处把"键名与注册
    表对不上"如实告诉管理员（拼错即报警，而不是拼错静默生效/失效）。"""
    warnings = []
    try:
        from core.datasource_manager import DataSourceManager
        known_ds = {n.casefold(): n for n in
                    (DataSourceManager()._config or {}).keys()}
    except Exception:
        known_ds = {}
    for k in (rules.get("datasources") or {}).keys():
        if known_ds and k.casefold() not in known_ds:
            warnings.append(f"数据源键 '{k}' 与已注册数据源对不上（已注册: {sorted(known_ds.values())}）")
    try:
        from core.auth import _get_conn
        rows = _get_conn().execute("SELECT username FROM users").fetchall()
        known_users = {r[0] for r in rows}
    except Exception:
        known_users = set()
    if known_users:
        known_cf = {u.casefold() for u in known_users}
        for k in (rules.get("users") or {}).keys():
            if k.casefold() not in known_cf:
                warnings.append(f"用户键 '{k}' 与 users 表对不上（现有用户: {sorted(known_users)}）")
    return warnings


@router.put("/api/permissions")
def put_permissions(body: dict, request: Request):
    """写入权限规则：结构校验 → 试载 → 原子写（自动备份）→ 立即生效。
    仅 admin + 操作密码（权限是全部防线的地基，改写必须人因确认）"""
    _require_admin(request)
    ...
    warnings = _spelling_warnings(rules)
    from core.config_hub import write_yaml_atomic
    write_yaml_atomic(_perm_path(), rules, backup=True)
    resp = {"ok": True, "message": "权限规则已保存并立即生效（无需重启）"}
    if warnings:
        from core.logger import get_logger
        for w in warnings:
            get_logger(__name__).warning("权限规则拼写核对: %s", w)
...
def test_permission(body: dict, request: Request):
    ...
    # 级联判定（与 check() 同源 first_deny：上级禁止下级不可解禁）
    denied = pol.first_deny(ds, operation, table)
    ...
    if not allowed:
        ...
        reason = (f"数据源 '{ds}'" + (f" 的表 '{table}'" if table else "")
                  ...
                  + f"（{mode} 模式{pol._mode_hint(mode, denied['allow'], denied['deny'])}）")
    return {"allowed": allowed, "scope": denied["scope"] if denied else "", "mode": denied["mode"] if denied else "", "reason": reason}
