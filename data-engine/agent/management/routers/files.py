...
- 单文件最大 100MB，单批总量最大 500MB，单批最多 200 个文件
"""
...
from agent.management.deps import logger, settings, _project_root
...
async def upload_files(request: Request):
    """批量上传文件落盘——登录用户（readonly 拒；评审五轮：2GB/批写盘无闸是
    DoS 面 + 内容进向量库的扩散面）
    ...
    """
    from agent.management.routers.backup_export import _require_user
    ...
    for i, item in enumerate(files):
        ...
        if len(data) > _MAX_FILE_BYTES:
            raise HTTPException(
                status_code=400,
                detail=f"{rel}: 文件过大（{len(data) // (1024 * 1024)}MB > 100MB）")
        total_bytes += len(data)
        if total_bytes > _MAX_BATCH_BYTES:
            raise HTTPException(status_code=400, detail="单批总量超过 500MB")
