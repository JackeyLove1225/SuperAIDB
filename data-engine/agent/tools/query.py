...
def aggregate_query_tool(table="", agg_func="COUNT", agg_field="*", group_by="", having="", where="", database=""):
    ...
    return aggregate_query(table=table, agg_func=agg_func, agg_field=agg_field,
                           group_by=group_by, having=having, where=where)
...
def _query_with_fallback(query="", table="", column="", conditions="", database="", page=1, page_size=100):
    ...
    # P1-4：关键词短路分支已删除——意图路由是决策树的工作（白盒），
    ...
    if not conditions and query:
        ...
        if not _s.FC_AI_ENABLED:
            ...
            try:
                ...
                return ToolResult.fail(f"查询条件解析失败: {e}", code="VALIDATION",
                                       reason="conditions_parse_failed", table=table)
    if table:
        from core.data_ops import _get_driver
        drv = _get_driver()  # FederatedDriver：自动路由到表所属数据源
        ...
        if conditions:
            try:
                ...
                # 由 build_where 自动归并为 IN——query 归一化补丁 20260803
        ...
        try:
            ...
            return ToolResult.fail(f"查询条件不安全: {e}", code="CONTRACT",
        ...
        if cond_sql:
            ...
            try:
                ...
                return ToolResult.fail(f"WHERE 条件不安全，已拒绝执行: {e}",
        ...
        try:
            ...
            return ToolResult.fail(f"查询总数统计失败: {e}", code="UNKNOWN",
                                   reason="count_failed", table=table)

        # 分页查询——query() 返回 list[dict]
        sql = 'SELECT * FROM "' + table + '"' + cond_sql + f' LIMIT {page_size} OFFSET {offset}'
        rows_dict = drv.query(sql)
        # 字段列表通过 get_columns() 获取（跨驱动标准接口）
        cols = [c["name"] for c in drv.get_columns(table)]

        if rows_dict:
            from core.db_chat import DBChat
            chat = DBChat()
            sid = get_context().save_selection(table, rows_dict, query or "")
            result = chat._format_multi_table({table: rows_dict})
