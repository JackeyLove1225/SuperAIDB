...
def process_file(filepath="", page_start=1, page_end=0, overwrite=False, database="", batch_size=TIER2_BATCH_UNITS, tables=""):
    ...
    if not filepath:
        ...
        if not filepath:
            return ToolResult.fail("请先上传文件，或在指令中指定文件路径",
    ...
    if filepath and not Path(filepath).exists():
        ...
        if len(candidates) > 1:
            # P1-12：同名多版本不静默选最新——报错列出候选，让 AI/用户明确指定完整路径
    ...
    from pipeline.runner import run as _run, _check_file_size
    ...
    try:
        result = _run(filepath, industry=settings.INDUSTRY, page_start=ps,
                      page_limit=page_limit, batch_size=batch_size, overwrite=overwrite,
                      only_tables=only_tables)
    except ValueError as e:
        return ToolResult.fail(f"处理失败: {e}", code="VALIDATION",
                               reason="pipeline_rejected")
    except Exception as e:
        return ToolResult.fail(f"处理异常: {str(e)[:200]}", code="UNKNOWN",
    ...
    # 未映射项（通用管线：源键/表头无法映射到任何表字段——如实列出，不入库不静默）
    ...
    if unmapped:
        ...
        # 挂起待确认映射：附系统猜测（用户回"对/忽略/是X表Y字段"即存档补录）
        try:
    ...
    if not any_new and not _failed:
