...
途径二：MCP 能力面（对 AI）
  ...
  mcp_server.py —— 全部数据能力经 MCP 协议暴露，高危操作双层人审
        │
        ▼  两条途径共用同一能力内核
  agent/tools/ —— 38 个原子工具 + 决策树路由（按域分包：query/records/ddl/files/templates/admin，__init__.py 为 facade，简单指令不依赖 AI 也可执行）
        │
        ▼
...
- **全量安全测试**：SQL 注入/危险语句拦截、权限矩阵、认证全链路、加密边界、daemon 隔离、人审双通道均有专项回归层（test_14/17/21/34/35/32 等），CI 每个 push 真跑

## 部署安全边界（数据主权边界 = 加密边界）

**边界内（接入本产品的库）我负责**：静态密文（SQLCipher AES-256，密钥由 OS 凭据管理器保管）+ 运行时数据层收进独立守护进程（daemon，密钥只驻其内存，任何其他进程只见 IPC 令牌）+ 全部操作过契约/权限/人审。

- **接入即加密**：明文 SQLite 库在注册/打开瞬间自动迁移为密文（迁移验证通过后默认删除明文备份，`MIGRATE_KEEP_PLAIN_BACKUP=1` 可保留排障）；`DB_ENCRYPT=false` 可显式回退（不推荐）
- **运行时隔离（默认开）**：`DAEMON_MODE=true` 时所有驱动调用经 `core/daemon/` 进程（29 接口 RPC 代理，上层零感知）；密钥驻留 daemon 内存，不落盘不进其他进程
...
**诚实声明的残差**：同一 OS 账号下的管理员级蓄意攻击（注入/调试 daemon 进程取内存密钥）不在防护范围——这是所有同类软件的共同底线，三期跨账号部署才覆盖。
...
agent/          # 编排层：open_layer（意图理解/任务拆解/执行/综合）、decision_tree、
                #   tools/（38 工具注册，按域分包：query/records/ddl/files/templates/admin）、
