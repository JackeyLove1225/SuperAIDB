...
class Settings:
    ...
    def validate(self):
        ...
        if missing:
            raise RuntimeError(
                f"配置缺失: {', '.join(missing)}。"
                f"请创建 data-engine/config/.env 文件"
            )
    ...
    # P1 语义解析 AI 开关：True=启用 P1 的 AI 语义解析（单步模式兼容）
    # False=默认关闭，走 LangGraph 结构化标签透传，跳过 P1 的 AI 解析
    P1_AI_ENABLED: bool = os.getenv("P1_AI_ENABLED", "false").lower() == "true"

    # FC（Function Calling）AI 开关：True=启用 FC AI 从自然语言提取参数（单步模式兼容）
    # False=默认关闭，走 LangGraph 输出的 structured_args JSON 直接构造参数，跳过 FC AI
