...
def clear_database(drop_tables: bool = False) -> str:
    ...
    if drop_tables:
        ...
        try:
            ...
            # 此前按 list_tables() 顺序删，父表（被引用方）先删必被契约拦截，
            # "删除数据库所有表格"在有 FK 的库上必现失败（20260804 修复）
        ...
        # 用户/AI 的安全闸，不该拦管理面维护操作）。此前 drv.delete(t,"1=1")
        # 被契约拒绝后 except 吞掉 → 静默清 0 表（评审四轮 L 修复）
        from core.contract.security_contract import safe_table_sql
        count = 0
        skipped = []
        conn = _sm._unwrap_sqlite_conn(drv)
        # 内置认证表（users）不清——清了等于把自己锁门外（与安全闸同口径）
        for t in [t for t in drv.list_tables()
                  if not t.startswith("sqlite_") and t != "users"]:
            try:
                if conn is not None:
