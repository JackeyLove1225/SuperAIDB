...
def batch_create_tables(definitions: list, overwrite: bool = False) -> str:
    ...
    results = []
    # 批处理窗：N 表批量建只触发一次三层对账（曾每表一次全量
    # reconcile+整目录快照，N 表 = O(N²) 元数据写放大）。finally 保证中途
    # 失败也把已发生的写对账出去。
    from core.registry import begin_batch, end_batch
    begin_batch()
    try:
        for name in ordered:
            ...
            if ds_name:
                try:
                    ...
                    # 表已建但联邦路由注册失败——数据可直查但跨库可能找不到，必须留痕
                    logger.warning("建表后注册数据源映射失败（联邦路由或缺该表）: %s", e)
        ...
        def _rollback_extra():
            ...
            for path, content in _deleted_holder.get("deleted", {}).items():
                try:
                    ...
                    # 回滚路径同样原子写（回滚必须比正向更可靠，
                    # 裸 write_text 让崩溃窗口截断的恰是正在恢复的文件）
                except Exception as e:
...
def modify_column(table: str, column: str, new_type: str, force: bool = False) -> dict:
    ...
    found["type"] = nt
    if not force:
        # 类型风险预检与执行闸同源（与契约层 TypeContract 共用同一套判定，
        # 避免双规则漂移——预检报"需 force"而契约层判 safe 的矛盾）
        from core.contract.type_contract import TypeContract
        risk = TypeContract.classify_change_risk(old_type, nt)
        if risk.level in ("warning", "danger") and risk.requires_force:
            found["type"] = old_type
            # 显式 need_force 自报（双轨：execute_tool 据此弹 force 确认卡）
            return {"ok": False, "need_force": True,
                    "message": risk.message or f"类型变更 {old_type} → {nt} 风险级 {risk.level}"}
...
def alter_precision(table: str, column: str, precision_str: str, force: bool = False) -> dict:
    ...
    if isinstance(old_prec, str):
        try: old_prec = tuple(int(p.strip()) for p in old_prec.split(","))
            ...
            old_prec = None  # 旧精度串解析失败按无精度处理（截断风险闸随之不触发——如实注释）
