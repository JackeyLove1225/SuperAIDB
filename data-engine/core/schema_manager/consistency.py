...
def _all_industry_yaml_tables() -> set:
    ...
    """
    from core.schema_matcher import _load_schemas as _load_schemas_all
...
def _check_tables(schema_dir, db_tables) -> tuple[set, str]:
    ...
    # 统一从规范入口加载（P2-5）；坏 YAML 显式抛错（U-1 R6，不再静默丢表）
...
def _check_indexes(tname, t, phys_conn) -> str:
    ...
    # 不得拿空集对比 YAML——会把"未校验"误报成"索引缺失"（daemon 模式实测事故）
...
def _preflight_check():
    ...
    if schema_empty:
        try:
            from core.data_ops import _get_driver as _get_fed_driver; drv = _get_fed_driver()
    ...
    from core.schema_matcher import _load_schemas as _load_schemas_deep
