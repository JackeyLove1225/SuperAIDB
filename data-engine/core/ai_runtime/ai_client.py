...
class AIClient:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None,
        ...
        # 显式超时：SDK 默认 600s/次——演示现场模型卡死不能干瞪眼
        _timeout = float(os.getenv("AI_TIMEOUT", "30"))
    ...
    def call_function(self, functions: list[dict], user_prompt: str,
        ...
        与 _call 同一重试收口（FC 通道不做单次裸调——
        模型抖动/限流时直接失败不可接受，必须有重试有降级）
        """
        messages = []
        if system_prompt:
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                ...
                if attempt < self.MAX_RETRIES:
                    delay = self.RETRY_DELAY * (2 ** (attempt - 1))
                    log_warning("AI FC 调用失败，准备重试", model=self.model,
                                attempt=attempt, error=str(e)[:60], delay=delay)
                    time.sleep(delay)
                else:
                    log_error("AI FC 调用最终失败", model=self.model,
                              error=str(e)[:100])
                    raise RuntimeError(
                        f"AI 服务不可用（重试{self.MAX_RETRIES}次仍失败）: {last_err}") from e
