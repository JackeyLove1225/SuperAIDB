...
def _cache_get(file_path: str) -> Optional[ParsedDocument]:
    ...
    加固：缓存改用 JSON 序列化（原 pickle.load 是 RCE 向量）。
...
def _cache_set(file_path: str, doc: ParsedDocument) -> None:
    ...
    加固：改用 JSON 序列化。若文档含非 JSON 可序列化字段
            ...
            for page in doc:
                ...
                if self.extract_tables:
                    try:
                        ...
                        get_logger(__name__).debug("PDF 页表格提取失败（保留文本通道）", exc_info=True)
