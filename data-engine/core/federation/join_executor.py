...
def _all_schema_tables() -> list[str]:
    ...
    try:
        ...
        return [t["name"] for t in load_schemas()]
    except Exception:
        return []
...
def _split_where_by_table(where: str, tables: list[str]) -> dict[str, str]:
    ...
    # 简化处理：按顶层 AND 拆分（sql_lex 词法器唯一实现——字面量内的
    # AND 不切；不支持复杂 OR 逻辑的拆分）
    from core.sql_lex import split_top_and_or
    conditions = split_top_and_or(where)
...
def _apply_cross_filter(rows: list[dict], cross_condition: str, tables: list[str]) -> list[dict]:
    ...
    filtered = []
    from core.sql_lex import split_top_and_or
    conditions = split_top_and_or(cross_condition)
...
def _evaluate_condition(row: dict, cond: str) -> bool:
    ...
    无法解析的条件如实报错（曾默认 return True——过滤器
    fail-open，用户拿到未过滤的结果且零披露，静默错比报错危险得多）
    """
    ...
    if not m:
        raise ValueError(f"跨表条件无法评估（仅支持 表.字段 比较符 值 的简单形态）: {cond[:60]}")

    field_key = f"{m.group(2)}.{m.group(3)}"
    op = m.group(4)
    raw_rhs = m.group(5).strip()
    ...
    # 右值是列引用（t2.col 形态）则取该行对应列的值——
    # 列对列比较按字面量字符串错评会结果偏空且零披露
    rhs_ref = re.match(r'^["`]?(\w+)["`]?\.["`]?(\w+)["`]?$', raw_rhs)
    if rhs_ref:
        ref_key = f"{rhs_ref.group(1)}.{rhs_ref.group(2)}"
        if ref_key not in row:
            raise ValueError(f"跨表条件右值列不存在: {ref_key}（行内可用列: {sorted(row)[:8]}…）")
        value = row[ref_key]
    else:
        value_str = raw_rhs.strip('"').strip("'")
        # 类型转换
        try:
    ...
    if op == "=":
        ...
        # 右值为列引用时 value_str 未定义——LIKE 模式源取右值实际值
        #（列对列 LIKE 会抛 UnboundLocalError，必须在此取实值）
...
def _fetch_main_side(main_table: str, where_parts: dict) -> tuple[Optional[list[dict]], Optional[str]]:
    """主表取数：带主表的 WHERE 条件查询，失败或空表返回错误消息

    Returns:
        (行列表, None) 成功；(None, 错误消息) 失败或无匹配数据
    """
    ...
    if err:
        return None, err
    if not main_rows:
        return None, f"主表 {main_table} 无匹配数据"
    return main_rows, None


def _join_transitive(
    jt: str,
    current_rows: list[dict],
    joined_tables: set,
    where_parts: dict,
    join_type: str,
) -> tuple[list[dict], Optional[tuple], Optional[str]]:
    """传递外键路径就地 JOIN：jt → 中间表 → 已JOIN的表

    例如 patient 和 diagnosis 无直接外键，但通过 visit 关联。
    先就地 JOIN mid_table（不动 join_list），再重新查找 jt 的关系。

    Returns:
        (current_rows, (relation, rel_with), None) 找到路径；
        (current_rows, None, None) 未找到；(current_rows, None, 错误消息) 取数失败
    """
    for mid_table, mid_rel_to_joined, mid_joined in _iter_transitive_paths(jt, joined_tables):
        if mid_table in joined_tables:
            continue  # 中间表已联（多路径候选去重）
        ...
        if relation:
            return current_rows, (relation, rel_with), None
    return current_rows, None, None


def _join_table_side(
    jt: str,
    current_rows: list[dict],
    joined_tables: set,
    where_parts: dict,
    ...
    """单侧 JOIN：取关联表数据并并入已 JOIN 结果

    查找外键关系：在所有已 JOIN 的表中查找与新表 jt 的直接外键关系，
    找不到时尝试传递路径；随后取数、确定 JOIN 方向并执行内存 JOIN。

    Returns:
        (current_rows, input_capped, None) 成功；
        (current_rows, input_capped, 错误消息) 失败
    """
    # 支持传递关系：c 关联 b（而非 main_table a）
    relation, rel_with = _find_direct_relation(jt, joined_tables)

    if not relation:
        current_rows, found, err = _join_transitive(
            jt, current_rows, joined_tables, where_parts, join_type)
        if err:
            return current_rows, input_capped, err
        if not found:
            return current_rows, input_capped, (
                f"未找到 {jt} 与已关联表({', '.join(sorted(joined_tables))})之间的外键关系，"
                f"无法自动推断跨库 JOIN 条件")
        relation, rel_with = found
    ...
    if err:
        return current_rows, input_capped, err
    if len(jt_rows) >= FEDERATED_JOIN_LIMIT:
        input_capped = True
    ...
    return current_rows, input_capped, None
...
def _apply_post_filters(
    ...
    """后处理：跨表 WHERE 过滤 → 限制结果集 → 选择字段（含列完整性回退）

    Returns:
        (结果行, 截断前总数, None) 成功；(None, 0, 错误消息) 跨表条件无法评估
    """
    # 应用跨表 WHERE 过滤
    if cross_condition:
        try:
    ...
    return current_rows, total_count, None


def _format_join_result(
    current_rows: list[dict],
    total_count: int,
    input_capped: bool,
    main_table: str,
    ...
    if input_capped:
        # 输入截断如实披露：某一侧取数顶到上限，结果基于该侧前 N 行计算，
        # 更大行表的匹配可能不完整（只在真命中时提示，小表不制造噪音）
        header += f"（说明：基于各表前 {FEDERATED_JOIN_LIMIT} 行计算，结果可能不完整）"

    formatted = format_multi_table({"跨库JOIN结果": current_rows})
    return f"{header}\n{formatted}"
...
def federated_join(
    ...
    main_rows, err = _fetch_main_side(main_table, where_parts)
    if err:
        return err

    # 输入截断命中标记：任一一侧取数顶到上限才如实披露（不命中不制造噪音）
    input_capped = len(main_rows) >= FEDERATED_JOIN_LIMIT

    # 3. 逐个关联表进行内存 JOIN
    # 初始化：主表数据加 table. 前缀
    current_rows = [
        {f"{main_table}.{k}": v for k, v in row.items()}
    ...
    for jt in join_list:
        if jt in joined_tables:
            # 传递路径内联 JOIN 时已处理的表（根治：旧实现往被迭代的
            # join_list 里 insert 中间表——迭代器按索引推进，当前表被二次
            # 处理，1:N 扇出平方、静默错数。绝不原地插表，重复即跳过）
            continue
        current_rows, input_capped, err = _join_table_side(
            jt, current_rows, joined_tables, where_parts, join_type, input_capped)
        if err:
            return err

    # 4. 跨表过滤 → 截断 → 字段选择
    current_rows, total_count, err = _apply_post_filters(
        current_rows, cross_condition, all_tables, select_fields)
    if err:
        return err

    # 5. 格式化结果
    return _format_join_result(current_rows, total_count, input_capped, main_table, join_list, dsm)
