...
def load_schemas(schema_dir=None) -> list:
    ...
    try:
        ...
        # 注册失败=联邦路由缺映射（多数据源时会找不到表）——必须留痕
        from core.logger import get_logger
        get_logger(__name__).warning("schema 注册到数据源映射失败（联邦路由或缺表）: %s", e)
    return tables


# 兼容别名：测试与存量调用 patch/引用 _load_schemas（patch 面不迁移）
_load_schemas = load_schemas


def load_table_schema(table: str) -> dict | None:
    """加载单张表的 schema YAML——规范加载入口（收敛点）。
...
# ── 依赖倒置自注册：驱动面的 schema 知识钩子（drivers/base.py 的
# _UNIQUE_KEY_RESOLVER / _TABLE_SCHEMA_LOADER）——实现模块 import 即接线，
# 直构造驱动的单测场景同样生效；DataSourceManager.__init__ 的重复注册幂等无害
from core.drivers.base import register_unique_key_resolver, \
    register_table_schema_loader as _register_tsl

register_unique_key_resolver(get_unique_key_column)
_register_tsl(load_table_schema)
