...
class DaemonDriver(Driver):
    ...
    def _call(self, method: str, **kwargs):
        from core.permission import get_effective_role, get_current_user
        from core.operator_gate import capability_present_for_rpc
        ...
        args = {**kwargs, "datasource": self._ds, "_role": get_effective_role(),
                "_user": get_current_user(), "_op_cap": capability_present_for_rpc()}
        # _role/_user/_op_cap 必须写在 **kwargs 之后：调用方同名 kwarg
        # 不得顶掉权限身份（旧序写在 kwargs 之前，等于给调用方留了覆盖口）
        try:
