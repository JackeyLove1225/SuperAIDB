...
- 方法白名单 = BaseDriver 29 接口（ping 在内）；调用按数据源名路由到真实驱动
...
class _Daemon:
    SESSION_IDLE_TTL = 30 * 60  # 会话驱动 30 分钟无调用回收（防长驻僵尸连接堆积）

    def __init__(self):
        self._dsm = None          # DataSourceManager（惰性，首调用时加载配置）
        self._drivers = {}        # ds_name → 真实驱动
        self._last_used = {}      # (数据源, 会话) → 最近调用时间戳（idle TTL 依据）
        self._cfg_mtime = -1.0    # 加载配置时 datasources.yml 的 mtime
        self._lock = threading.Lock()
        self._busy = False        # 工作线程执行中标志（ping 应答携带：忙≠死可区分，
                                  # ping 快速通道不能让卡死检测彻底消失）

    def _reap_idle_sessions(self, now: float, keep_key=None):
        """回收 idle 超期的会话驱动（调用驱动时顺手扫，零额外线程）"""
        for k, ts in list(self._last_used.items()):
            if k != keep_key and now - ts > self.SESSION_IDLE_TTL:
                d = self._drivers.pop(k, None)
                self._last_used.pop(k, None)
                if d is not None:
                    try:
                        d.close()
                    except Exception:
                        pass  # 会话驱动回收关闭失败——进程退出时 OS 回收
    ...
    def _driver(self, ds_name: str, session: str = ""):
        ...
        # 不许再持同一把锁（非重入锁持锁重入会自锁死锁，20260822）
        # 配置新鲜度：datasources.yml 变了就整体重载并清驱动缓存
        #（防旧路径句柄复活——跨启动重注册同名数据源时按名字命中旧句柄）
        try:
            import os as _os
        ...
        if self._dsm is not None and mtime != self._cfg_mtime:
            ...
            for d in self._drivers.values():
                try:
                    ...
                    pass  # 配置变更时关旧驱动失败——文件锁随进程/句柄回收兜底
            self._dsm = None
            self._drivers.clear()
        ...
        self._reap_idle_sessions(now, keep_key=key)  # idle TTL 顺手扫（零额外线程）
        if key in self._drivers:
            return self._drivers[key]
        if self._dsm is None:
            from core.datasource_manager import DataSourceManager
            # 显式实例化：绕开单例——单例的驱动缓存按名字命中旧路径句柄
            #（跨启动重注册同名数据源时报 "unable to open" 的根因）
    ...
    def handle(self, req: dict) -> dict:
        ...
        # 注：ping 永远到不了这里——IO 线程快速道（_read_loop）验令牌后直接应答，
        # 不排工作线程的队；_ALLOWED 里保留 "ping" 仅作协议文档
        ...
        # 认证/授权在 mgmt（Bearer/API-Key 路由级校验）与 MCP（MCP_USER 用户绑定 + 提权审批契约）层。
        ...
        try:
            ...
            from core.permission import set_current_role, set_current_user
            from core.operator_gate import rpc_capability_grant
            if role:
                set_current_role(role)
            if user:
                set_current_user(user)  # 用户级/自助规则 daemon 侧同判定
            try:
                # session 亲和：同 session 的调用在同一驱动实例内串行（事务正确性）
                # 能力传递：调用方持有效凭证 → 本调用期 daemon 侧授能力（信任
                # 模型同 _role/_user——daemon 是隔离边界不是认证边界）
                self._busy = True
                with self._lock, rpc_capability_grant() if op_cap else _nullctx():
                ...
                if method == "close":
                    ...
                    for k in [k for k in self._drivers if k[1] == (session or "default")]:
                        try:
                            ...
                            pass  # 会话驱动关闭失败——进程退出时 OS 回收
                        self._drivers.pop(k, None)
                        self._last_used.pop(k, None)  # idle 台账同步清
            finally:
                self._busy = False
                if user:
                    set_current_user("")
...
def main():
    ...
    # 环境契约自检（fail-fast）：daemon 必须在 DAEMON_MODE=false 下
