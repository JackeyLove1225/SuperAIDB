...
def _checkpoint_wal(path: str) -> None:
    ...
    此前从不读返回值：别的连接持库时 busy=1 静默跳过，export 只导主文件，
    WAL 尾部数据永久丢失（安全评审实测发现）。
    """
    wal = str(path) + "-wal"
...
def migrate_to_encrypted(path: str, key: str) -> None:
    ...
    try:
        ...
        src.execute(f"ATTACH DATABASE '{tmp}' AS encrypted KEY '{key}'")
...
def migrate_to_plaintext(path: str, key: str) -> None:
    ...
    try:
        ...
        src.execute(f"ATTACH DATABASE '{tmp}' AS plaintext KEY ''")
