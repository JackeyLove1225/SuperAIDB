...
选择集（selections）文件化（20260822 修复；迁移到 JsonContract
...
# 通道/轨迹/批量预批准改 ContextVar：单例字段在 anyio.to_thread 工作线程间
# 并发互踩——A 的 finally 会把 B 执行中途的通道改回 graph，trace 同帧互踩；
# nuke_batch 同理（A 结算清场会把 B 执行中途的预批准抹掉）
_CHANNEL_VAR = _contextvars.ContextVar("channel", default="graph")
_TRACE_VAR = _contextvars.ContextVar("trace_id", default="??")
_NUKE_BATCH_VAR = _contextvars.ContextVar("nuke_batch", default=None)
...
class ContextManager:
    ...
    def __init__(self):
        ...
        # channel/trace_id/nuke_batch 不在实例字段——由模块级 ContextVar 承载
        #（并发 MCP 调用经 anyio.to_thread 各见各的）
    ...
    def clear_all(self):
        """清空全部会话态（暂存上下文+选择集+批量预批准）——行业切换时调用。
        选择集是文件化共享态：必须落盘清空，否则其他进程仍读到旧行业选择集"""
        self._context = None
        self._selections = {}
        self._save_selections()
        _NUKE_BATCH_VAR.set(None)

    def set_nuke_batch(self, tables: set, ops: set):
        """设置批量预批准——mutate_natural 多表合并卡已获用户批准，
        逐表 execute_tool 时人审闸免检（避免同一批准重复弹卡）。
        ContextVar 承载：并发调用各自持有各自的预批准，finally 清场不互踩"""
        _NUKE_BATCH_VAR.set({"tables": set(tables), "ops": set(ops)})

    def get_nuke_batch(self) -> dict | None:
        """获取批量预批准（无则 None）"""
        return _NUKE_BATCH_VAR.get()
    ...
    def set_channel(self, channel: str):
        """设置调用通道：graph（默认）/ mcp（挂起表回执）——ContextVar 承载：
        anyio.to_thread 会把 contextvars 复制进工作线程，并发 MCP 调用各见
        各的通道（单例字段会互踩）"""
    ...
    def set_trace_id(self) -> str:
        """生成并存储请求追踪 ID（ContextVar——并发调用轨迹不互踩）"""
        self._trace_id = f"req-{uuid.uuid4().hex[:8]}"
        _TRACE_VAR.set(self._trace_id)
        return self._trace_id

    def _current_owner(self) -> str:
        """选择集主体（会话隔离）：channel:进程:角色——
        两个并发 MCP 客户端会话（两个 MCP 进程）不共享"最近选择集"，
        杜绝 A 会话误批 B 会话的数据"""
        import os as _os
        try:
            from core.permission import get_current_role
            role = get_current_role()
        except Exception:
            role = ""
        return f"{_CHANNEL_VAR.get()}:{_os.getpid()}:{role}"
    ...
    def get_last_selection_id(self):
        """获取本主体最近创建的选择集编号（主体隔离）；
        本主体无选择集返回 None——不再跨主体"最近"串台"""
        owner = self._current_owner()
        mine = [int(k) for k, v in self._load_selections().items()
                if v.get("owner") in (None, "", owner) or not v.get("owner")]
        # 兼容存量无 owner 字段的选择集（历史文件），但本主体优先
        mine_own = [k for k, v in self._load_selections().items()
                    if v.get("owner") == owner]
        pool = mine_own or mine
        if not pool:
            return None
        return max(pool)

    def list_selections(self) -> list[dict]:
        """列出本主体的选择集摘要（读路径同样按 owner 过滤——与写路径
        （edit/delete owner 校验）同口径：跨会话的表名/条件/样本行不回读）"""
        owner = self._current_owner()
        result = []
        for sid, sel in self._load_selections().items():
            if sel.get("owner") and sel.get("owner") != owner:
                continue  # 其他主体的选择集不出示
            result.append({
                "id": int(sid),
                "table": sel["table"],
...
# 自注册到重置注册表：行业切换清空会话态（暂存+选择集+批量预批准）
from core.registry import register_reset

register_reset("context", lambda: get_context().clear_all())
