...
设计原则：
- 上层通过 Steward.get_driver() 拿到 ContractDriver 实例，零感知
...
- 兼容方法（1）：_safe_where → 委托 SecurityContract
  （原 _translate_error 兼容空壳已删——全仓零调用方，错误翻译统一 ErrorTranslator）
合计覆盖 27 抽象 + 1 兼容（_safe_where）= 全部接口。
"""
import inspect
from core.logger import get_logger
from core.operator_gate import require_capability as _require_operator_cap
import re
...
from .security_contract import (SecurityContract, extract_table_aliases,
                                extract_tables_from_sql, safe_column_sql,
                                split_set_pairs)
...
from core.permission import Operation, PermissionPolicy, PermissionDenied
from core.permission.sql_guard import resolve_datasource, guard_write_sql

# 对象归属数据源解析钩子（依赖倒置）：drop_index 等无表维度操作需要
# "对象在哪个数据源"的判定——该知识在 DataSourceManager（上层），契约层
# 不向上 import，改由上层实例化时注册本钩子；未注册时退化默认库语义
#（直接构造 ContractDriver 的独立测试场景）
_OBJECT_DS_RESOLVER = None


def register_object_ds_resolver(fn) -> None:
    """注册对象归属数据源解析器（DataSourceManager.__init__ 调用）"""
    global _OBJECT_DS_RESOLVER
    _OBJECT_DS_RESOLVER = fn
...
# 系统表清单（用户管理面资产，20260903 收口为模块级唯一真源）：
# 认证/授权/会话表的一切 DML 只能经 core.auth 官方函数（register_user/
# delete_user/update_user_role——带业务不变量保护）或管理端 HTTP 端点
# （admin Bearer + 操作密码闸）。数据面（execute SQL/记录级 DML 工具）
# 一律拒绝——SQL 直改绕过"最后一名 admin 保护/密码哈希规范"等不变量。
_SYS_TABLES = frozenset({
    "users", "roles", "permissions", "role_permissions", "sessions",
})


def _is_system_table(tbl: str) -> bool:
    """系统表/内部表判定（大小写归一化——SQLite 表名大小写不敏感）"""
    t = str(tbl or "").lower()
    return t in _SYS_TABLES or t.startswith(("sqlite_", "meta_"))
...
def _validate_execute_sql(sql) -> None:
    ...
    # 识别语句目标表，系统表一律拒绝。
    # 目标表名捕获段（三种形态，DDL/DML 同源）：
    # 裸标识符 / 引号包裹("name"、'name'、`name`) / SQLite 方括号([name]) /
    # schema 前缀(main.users、[main].[users]) —— 取最后一个捕获段（末段表名）
    _TBL_SEG = (r"(?:[\"'`\[]([a-zA-Z0-9_]+)[\]\"'`]|([a-zA-Z0-9_]+))"
                r"(?:\s*\.\s*(?:[\"'`\[]([a-zA-Z0-9_]+)[\]\"'`]|([a-zA-Z0-9_]+)))?"
                r"(?:\s*\.\s*(?:[\"'`\[]([a-zA-Z0-9_]+)[\]\"'`]|([a-zA-Z0-9_]+)))?")

    def _deny_if_system(pattern: str, what: str) -> None:
        m = re.search(pattern, sql, re.IGNORECASE)
        if m:
            _parts = [g for g in m.groups() if g]
            _tbl = (_parts[-1] if _parts else "").lower()
            if _is_system_table(_tbl):
                raise SecurityError(
                    f"execute() 禁止{what}系统表: {_tbl}——"
                    f"用户/权限管理请走 Web 管理台（admin + 操作密码确认）")

    if first in ("DROP", "ALTER"):
        # DDL：DROP/ALTER TABLE <name>（结构面）
        _deny_if_system(
            r"\b(?:DROP|ALTER)\s+TABLE(?:\s+IF\s+EXISTS)?\s+" + _TBL_SEG,
            "结构变更")
    elif first == "INSERT":
        # DML：INSERT INTO <t>（数据面——SQL 直改 users 绕过 core.auth
        # 业务校验：最后一名 admin 保护/密码哈希规范，20260903 收口）
        _deny_if_system(r"\bINSERT\s+(?:OR\s+\w+\s+)?INTO\s+" + _TBL_SEG, "写入")
    elif first == "UPDATE":
        _deny_if_system(r"\bUPDATE\s+" + _TBL_SEG, "更新")
    elif first == "DELETE":
        _deny_if_system(r"\bDELETE\s+FROM\s+" + _TBL_SEG, "删除")


class ContractDriver(Driver):
    """契约驱动——包装原始 Driver，强制经过所有契约校验

    Steward.get_driver() 透明返回本类的实例，上层无感知。
    ...
    def _perm(self, table: str, op, method: str) -> None:
    ...
    @staticmethod
    def _sys_table_deny_msg(table: str, what: str) -> str:
        """系统表 DML 拒绝文案（四处共用同一口径）"""
        return (f"表 {table} 是系统表（用户/权限管理面资产），数据面禁止{what}——"
                f"增删用户/改角色请走 Web 管理台「用户管理」"
                f"（admin + 操作密码确认），AI 通道发起将转人工审批")

    def insert(self, table: str, rows: list, overwrite: bool = False) -> dict:
        """批量插入——经 DataCrudContract.validate_insert

        overwrite 语义含内部 DELETE（物理驱动先删冲突旧行）——权限判定
        单点收口（三调用路径曾各修各的，单库主路径漏网）：
        overwrite=True 时 INSERT 与 DELETE 同查，"只许写不许删"的角色
        无法借 overwrite 删除既有记录。
        """
        # 系统表豁免（20260903）：用户/权限表的 DML 只走 core.auth 官方函数
        if _is_system_table(table):
            raise SecurityError(self._sys_table_deny_msg(table, "写入"))
        _require_operator_cap("insert")
        try:
            cleaned = DataCrudContract.validate_insert(self._driver, table, rows)
            self._perm(table, Operation.INSERT, "insert")
            if overwrite:
                self._perm(table, Operation.DELETE, "insert(overwrite)")
            if cleaned:
    ...
    def update(self, table: str, set_clause: str, where: str = "") -> dict:
        ...
        if _is_system_table(table):
            raise SecurityError(self._sys_table_deny_msg(table, "更新"))
        _require_operator_cap("update")
        try:
            DataCrudContract.validate_update(self._driver, table, set_clause, where)
            self._perm(table, Operation.UPDATE, "update")
            # 全仓唯一 SET 解析器提取列（权限判定与驱动执行同源）
            self._perm_columns(
    ...
    def delete(self, table: str, where: str) -> dict:
        ...
        if _is_system_table(table):
            raise SecurityError(self._sys_table_deny_msg(table, "删除"))
        _require_operator_cap("delete")
        try:
    ...
    def delete_by_pk(self, table: str, pk_column: str, pk_value) -> dict:
        """按主键删除（接口契约）——经 DataCrudContract.validate_delete 等价校验"""
        if _is_system_table(table):
            raise SecurityError(self._sys_table_deny_msg(table, "删除"))
        _require_operator_cap("delete_by_pk")
        try:
    ...
    def _check_read_perm_and_mask(self, sql: str) -> str:
        ...
        for t in tables:
            ...
            # WHERE password_hash LIKE 'a%' 成为逐位拖库的布尔预言机
            for col in denied:
    ...
    def drop_table(self, table: str) -> str:
        ...
        _require_operator_cap("drop_table")
        try:
    ...
    def drop_column(self, table: str, column: str) -> dict:
        """删字段——主键保护 + 外键引用检查"""
        _require_operator_cap("drop_column")
        try:
    ...
    def modify_column(self, table: str, column: str, new_type: str, force: bool = False) -> dict:
    ...
    def drop_foreign_key(self, table: str, constraint_name: str) -> dict:
        """删外键——标识符校验"""
        _require_operator_cap("drop_foreign_key")
        try:
    ...
    def drop_index(self, name: str) -> str:
        """删索引——标识符校验 + 目标描述符同源判定"""
        _require_operator_cap("drop_index")
        try:
            # 对象归属解析走注册钩子（依赖倒置——契约层不向上感知
            # DataSourceManager）；未注册（独立构造场景）退化默认库语义
            if _OBJECT_DS_RESOLVER is not None:
                _ds, _st = _OBJECT_DS_RESOLVER(name, "index")
            else:
                _ds, _st = None, "not_found"
            # 判定域=对象实际所在数据源名，直接喂 policy.check（
            # 曾把数据源名当表名过 resolve_datasource——表映射里没有它，
            # 恒落回默认库，非默认库的库级规则整体失效）
            PermissionPolicy.get_instance().check(
                _ds or resolve_datasource(""), Operation.DROP, "drop_index", table="")
    ...
    def recreate_table(self, table_config: dict) -> dict:
        ...
        """
        _require_operator_cap("recreate_table")
        try:
    ...
    def execute(self, sql: str):
        ...
        # DROP 头属结构级不可逆——与 drop_table 同人因闸（脚本直调无凭证拒）
        _head = sql.lstrip().split(None, 1)[0].upper() if sql.strip() else ""
        if _head == "DROP":
            _require_operator_cap("execute(DROP)")
        # 权限护栏：裸 SQL 写/DDL 同样受 permissions.yml 约束（识别不了的读/管理语句放行）
        # 判定域=执行域：透传口的执行目标库=self._driver 实际承载的
        # 数据源（DaemonDriver._ds 或默认），无表维度的操作（drop_index/trigger/
        # view/reindex）按此域判定——不再按默认库猜
        guard_write_sql(sql, datasource=getattr(self._driver, "_ds", ""))
        try:
    ...
    def _get_column_type(self, table: str, column: str) -> str:
        ...
        try:
            ...
            pass  # 列类型查不到按 TEXT（最宽类型，校验取保守方向）
        return "TEXT"
...
def _is_precision_tightened(old_prec, new_prec) -> bool:
    """判断精度是否收紧（总长或小数位变小）——唯一实现委托 ChangeAnalyzer
    （同包双实现曾仅 except 分支措辞不同；本包装保留模块内既有调用面）"""
    return ChangeAnalyzer._is_precision_tightened(old_prec, new_prec)
