...
class ChangeAnalyzer:
    ...
    def _diff_column(
        ...
        Args:
            ...
            old_col: 旧字段配置
            new_col: 新字段配置
            drv: Driver 实例（可选，用于数据采样扫描）
            scan_table: 数据扫描用的表名（一般取旧表名，因为变更前数据在旧表下）
            report: 累积变更报告
        """
        # 主键变更检测
        ...
        if new_nn and not old_nn:
            data_impact = {}
            if drv is not None and scan_table:
                try:
                    # 防御性校验标识符
                    ...
                    null_count = rows[0].get("c", 0) if rows else 0
                    data_impact = {"null_count": null_count}
                except Exception:
                    pass
            report.changes.append(Change(
        ...
        if new_uniq and not old_uniq:
            ...
            if drv is not None and scan_table:
                try:
                    ...
                    rows = drv.query(dup_sql)
                    dup_count = rows[0].get("c", 0) if rows else 0
                    data_impact = {"duplicate_groups": dup_count}
                except Exception:
                    pass
            report.changes.append(Change(
