...
class MetaDB:
    ...
    def conn(self) -> sqlite3.Connection:
        """获取当前线程的 SQLite 连接（线程安全 + 死线程连接清扫）"""
        tid = threading.get_ident()
        # 死线程连接清扫：ident 复用后拿到死线程的连接会触发
        # sqlite3.ProgrammingError（check_same_thread）
    ...
    def _migrate_from_legacy_if_empty(self):
        """一次性迁移：共享 data_engine.db 中的 meta_* 数据迁入按行业文件。
    ...
    # ── schema 版本登记（升级链）──
    # 1 = 初始三表；2 = meta_columns 唯一/索引/CHECK 模板列（_migrate_meta_columns）
    SCHEMA_VERSION = 2
    # 未来迁移登记表：[(version, note, fn(conn))]——按版本升序逐条幂等执行
    MIGRATIONS: list = []

    def _ensure_schema_version(self, conn):
        """meta_schema_version 版本表：升级演练的锚点——新二进制对旧库能回答
        "这库是哪个版本建的、还差哪些迁移"，而不是靠猜"""
        conn.execute("""
        ...
        """)
        have = {r[0] for r in conn.execute("SELECT version FROM meta_schema_version")}
        if not have:
            conn.execute("INSERT INTO meta_schema_version (version, note) VALUES (1, '初始三表')")
        for ver, note, fn in self.MIGRATIONS:
            if ver not in have:
                fn(conn)  # 迁移函数须幂等（版本表记录的是"已应用"而非"已尝试"）
                conn.execute(
                    "INSERT INTO meta_schema_version (version, note) VALUES (?, ?)",
                    (ver, note))
        if self.SCHEMA_VERSION not in have and not self.MIGRATIONS:
            conn.execute(
                "INSERT OR IGNORE INTO meta_schema_version (version, note) VALUES (?, ?)",
                (self.SCHEMA_VERSION, "meta_columns 唯一/索引/CHECK 模板列"))
    ...
    def add_column_if_missing(self, table_name: str, column_name: str,
        ...
        service 层不得再直接摸 self.conn 手写 SQL（封装纪律）：
        元数据写只经本类方法。
        """
...
register_reset("meta_db", MetaDB.reset_instance)
