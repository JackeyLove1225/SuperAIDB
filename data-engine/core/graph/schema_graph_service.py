...
实际建表通过 Steward().get_driver().create_table() 执行。
"""
import threading
...
class SchemaGraphService:
    ...
    @staticmethod
    def _merge_table_updates(old_schema: dict, updates: dict, table_name: str) -> dict:
        """合并 updates 出 new_schema（update_table / precheck_update 唯一合并实现）

        深拷贝避免污染 old_schema；name 恒钉为 table_name。
        预览与执行必须走同一份合并语义——两处曾各抄一份导致漂移。
        """
        ...
        if "columns" in updates:
            # 业务元数据保护：前端编辑器模型按结构提交，不带/空带
            # business_name/description 时按字段名从旧 schema 继承——
            # 整体替换曾把全表业务名静默抹掉（元数据丢失，lint R2 抓红）；
            # 显式提供新值的仍以新值为准。
            old_cols = {c.get("name"): c for c in old_schema.get("columns", [])}
            merged_cols = []
            for col in updates["columns"]:
                old = old_cols.get(col.get("name")) or {}
                for k in ("business_name", "description"):
                    if not col.get(k) and old.get(k):
                        col = {**col, k: old[k]}
                merged_cols.append(col)
            new_schema["columns"] = merged_cols
    ...
    def update_table(self, table_name: str, updates: dict, force: bool = False) -> dict:
        ...
        # 合并出 new_schema（唯一实现：_merge_table_updates）
        new_schema = self._merge_table_updates(old_schema, updates, table_name)
        ...
        if has_schema_change:
            ...
            try:
                ...
                drv = Steward().get_driver(datasource)
    ...
    def _apply_column_changes(self, drv, table_name: str, report, new_schema: Optional[dict] = None) -> None:
        ...
        for change in report.changes:
            try:
                ...
                from core.exceptions import AppError
                raise AppError(
                    f"表结构变更失败（{change.target}）：{str(e)[:120]}"
                    f"（元数据未改动）") from e
    ...
    def precheck_update(self, table_name: str, updates: dict) -> dict:
        ...
        # 合并语义与 update_table 同源（_merge_table_updates 唯一实现）；
        # CHECK 预校验也必须同款——预览放过而执行拦截即语义分叉
        new_schema = self._merge_table_updates(old_schema, updates, table_name)
        check_err = self._normalize_and_validate_checks(new_schema.get("columns", []))
        if check_err:
            return {"ok": False, "message": check_err}
    ...
    def delete_table_precheck(self, table_name: str) -> dict:
        ...
        try:
            drv = Steward().get_driver(meta.get("datasource") or None)
        ...
        for fk in all_fks:
            ...
            try:
                ...
                src_drv = Steward().get_driver(src_meta.get("datasource") or None)
    ...
    def create_relationship(self, table_name: str, column_name: str,
        ...
        try:
            ...
            drv = Steward().get_driver(src_datasource)
    ...
    def reconcile_from_yaml(self) -> dict:
        """全量对账收敛：sync_from_yaml 补齐 + 清掉 YAML 已不存在的表（P-D）

        AI DDL 路径（tools/ddl → schema_manager）写完 YAML 后经
        registry 的 schemas_written 通道触发本方法——MetaDB/Ladybug 与 YAML
        始终同态，设计器图谱不再陈旧到下次重启。
        """
        result = self.sync_from_yaml()
        # 清掉 YAML 已不存在的表（删表/改名后的两层元数据残留）
        schemas_dir = _get_schemas_dir()
        live = {f.stem for f in list(schemas_dir.glob("*.yaml")) + list(schemas_dir.glob("*.yml"))}
        pruned = 0
        for t in self._meta.list_tables():
            name = t.get("name") if isinstance(t, dict) else t
            if name and name not in live:
                self._meta.delete_table(name)
                try:
                    LadybugStore.delete_table_node(name)
                except Exception as e:
                    log_warning("图库节点清除失败", table=name, error=str(e)[:60])
                pruned += 1
        if pruned:
            log_info("对账收敛：清除 YAML 外残留表", pruned=pruned)
        result["pruned"] = pruned
        return result

    def verify_reconciliation(self) -> dict:
        """三层对账：YAML ↔ MetaDB ↔ Ladybug 漂移检测。
    ...
    def _normalize_and_validate_checks(self, columns: list[dict]) -> str:
        ...
        """
        from core.check_templates import render_expr
    ...
    def _read_yaml(self, table_name: str) -> Optional[dict]:
        """读取 YAML schema 文件——薄委托 schema_matcher.load_table_schema（加载收敛）"""
    ...
    def _write_yaml(self, table_name: str, schema: dict):
        """写入 YAML schema（原子写 tmp+replace，与 schema_manager._shared 同一实现——
        P-E 纪律收敛：同一类工件不再三种写入纪律并存）"""
        from core.schema_manager import _atomic_write
        schema["name"] = table_name
        _atomic_write(
            _get_schema_path(table_name),
...
# 自注册到重置注册表：行业切换遍历即覆盖
#（其缓存的 MetaDB 引用按行业，漏重置会串台）
from core.registry import register_reset

register_reset("schema_graph_service", SchemaGraphService.reset_instance)

# 订阅 schemas_written 通道（P-D）：schema_manager 写完 YAML 即触发三层对账收敛
from core.registry import register_on_change

register_on_change("schemas_written", "graph_reconcile",
                   lambda: SchemaGraphService.get_instance().reconcile_from_yaml())
