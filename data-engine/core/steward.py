...
get_driver() 返回默认数据源的 Driver（向后兼容）。
"""
...
class Steward:
    ...
    def reset_instance(cls):
        """单例重置（公开入口）——行业切换必须触发：
        缓存的默认数据源驱动指向旧行业库，漏重置则 DDL 落错库
        （DDL/DML 新鲜度分裂的病型）"""
        cls._instance = None

    def get_driver(self, datasource: str = None):
        """获取数据源的 Driver（公开门面；已包装 ContractDriver）
...
# 自注册到重置注册表：行业切换遍历 registry 即覆盖本单例，
# 不再依赖切换点手工点名（漏点名=DDL 落旧行业库的病型）
from core.registry import register_reset

register_reset("steward", Steward.reset_instance)
