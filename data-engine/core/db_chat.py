...
def _build_select_sql(table: str, fields: str, where: str = "",
                      ...
                      limit: int = 0) -> str:
    """构建 SELECT SQL——薄委托 data_ops.build_select_sql（P2-5 查询路径收敛：
    拼装与校验的唯一实现点在 core/data_ops.py）"""
...
class DBChat:
    ...
    def _get_table_map(self) -> dict:
        """返回 {表名: {desc, columns}}（复用 schema_matcher._load_schemas 唯一加载入口）
    ...
    def _format_multi_table(self, results: dict[str, list[dict]]) -> str:
        ...
        for table_name, rows in results.items():
            ...
            # 多列 → Markdown 表格（排版策略读 schema YAML 的 display 配置，P1-2）
    ...
    def ask(self, question: str) -> str:
                        ...
                        "properties": {
                            "tables": {
                                ...
                                "items": {
                                    ...
                                    "properties": {
                                        "table": table_enum,
                                        "where": {"type": "string", "description": "[可选/向后兼容] WHERE 条件字符串。推荐使用 where_conditions。{fields_hint}"},
                                        "where_conditions": {
                                            "type": "array",
        ...
        if not tables_raw and fn_args.get("table"):
            ...
            # 编码格式读行业 config.yml 的 code_pattern（P1-2：不再硬编码业务编码字段名
