...
T1.1 安全加固：所有 f-string SQL 中的标识符插值在方法入口用
SecurityContract.validate_identifier 校验，非法标识符抛 SecurityError。
...
from core.contract.security_contract import SecurityContract, is_valid_identifier, safe_default_sql
...
# T1.3 收敛：标识符正则统一定义在 core.contract.security_contract（{0,63} 上限），
...
def _map_type(col_type: str) -> str:
    ...
    T1.2 加固：入参先做白名单/正则约束，只允许 `NAME` 或 `NAME(n)`/`NAME(m,n)`
...
class MysqlDriver(Driver):
    ...
    def conn(self):
        ...
        try:
            ...
            # 重连只有一个通道：_connect() 全量重建（评审三轮复核发现）
    ...
    def insert(self, table: str, rows: list[dict], overwrite: bool = False) -> dict:
        ...
        # T1.1: 入口校验 table 和所有字段名（防御性，确保 f-string SQL 安全）
    ...
    def update(self, table: str, set_clause: str, where: str = "") -> dict:
        # T1.2: SET 子句参数化——列名走标识符白名单校验，值走 %s 参数占位
        ...
        # 全仓唯一 SET 解析器（与 sqlite 驱动同源，评审四轮 S-2）
    ...
    def add_foreign_key(self, table: str, column: str, ref_table: str, ref_column: str = "id") -> dict:
        # T1.1: 入口校验 table, column, ref_table, ref_column
        SecurityContract.validate_identifier(table, "表名")
        SecurityContract.validate_identifier(column, "字段名")
    ...
    def drop_foreign_key(self, table: str, constraint_name: str) -> dict:
        # T1.1: 入口校验 table 和 constraint_name
        SecurityContract.validate_identifier(table, "表名")
    ...
    def create_index(self, table: str, columns: str, unique: bool = False) -> str:
        # T1.1: 入口校验 table 和 columns（逗号分隔的字段名列表，逐个校验）
        SecurityContract.validate_identifier(table, "表名")
        ...
        # T1.1: 校验生成的索引名
        SecurityContract.validate_identifier(idx_name, "索引名")
    ...
    def recreate_table(self, table_config: dict) -> dict:
        ...
        # T1.1: 校验生成的临时表名
        SecurityContract.validate_identifier(temp_name, "临时表名")
        cursor = self.conn.cursor()
        try:
