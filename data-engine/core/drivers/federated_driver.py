...
# 全局 DataSourceManager 提供者钩子（依赖倒置注册点；生产注入优先，
# 本钩子兜底 docstring 示例与测试的裸构造用法）
_DSM_PROVIDER = None


def register_dsm_provider(fn) -> None:
    """注册 DataSourceManager 提供者（DataSourceManager.__init__ 调用）"""
    global _DSM_PROVIDER
    _DSM_PROVIDER = fn
...
class FederatedDriver(Driver):
    ...
    def __new__(cls, *args, **kwargs):
        # 状态初始化放 __new__：测试常用 __new__ 裸构造注入 _dsm（绕 __init__），
        # 放 __init__ 会让 _dirty_drivers/_savepoint_stack 在那条路径上缺席
        inst = super().__new__(cls)
        inst._dirty_drivers = set()
        # 命名 savepoint 栈：begin(name) 压栈；驱动首次变脏时
        # 在其连接上按栈序懒开同名 savepoint——rollback(name) 才能在每个
        # 库上回滚到"自己的"同名点（旧实现只在默认库开点，非默认库的写
        # 落在假 savepoint 外：rollback 空转、commit 又把"已回滚"广播落库）
        inst._savepoint_stack = []
        return inst

    def __init__(self, dsm=None):
        # 数据源管理器注入（依赖倒置）：路由知识在 DataSourceManager（上层），
        # 驱动面不向上 import——生产路径由调用方显式传入；缺省走全局提供者
        # 钩子（DataSourceManager 实例化时注册），未注册则如实报错
        if dsm is None:
            if _DSM_PROVIDER is None:
                raise RuntimeError(
                    "FederatedDriver 需要 DataSourceManager 实例（构造注入或"
                    "经 DataSourceManager 实例化注册全局提供者）")
            dsm = _DSM_PROVIDER()
        self._dsm = dsm
        self._dsm.load_config()
        # （_savepoint_stack 等状态已在 __new__ 初始化——__init__ 不再重置）

    def _mark_dirty(self, drv: Driver):
        """登记执行过写操作的物理驱动，供 commit/rollback 逐一处理"""
        if drv not in self._dirty_drivers:
            for sp in self._savepoint_stack:
                drv.begin(sp)  # 懒开：把当前 savepoint 栈补开到该驱动连接上
    ...
    def query(self, sql: str) -> list[dict]:
        ...
        """
        from core.sql_safe import extract_tables_from_sql
        ...
        if fks and ds_name:
            ...
            if skipped_fks:
                ...
                get_logger(__name__).info(
                    "跳过跨数据源外键: %s → %s（仅在schema层面记录）",
                    table_name,
                    [(fk.get('references'), fk.get('columns')) for fk in skipped_fks])
    ...
    def drop_index(self, name: str) -> str:
        """删除索引——目标描述符路由（跨数据源定位索引所在库，
        路由与判定同源；旧实现只打默认库，对象不在默认库时静默误路由）"""
        ds_name, status = self._dsm.resolve_object_datasource(name, "index")
        drv = self._dsm.get_driver(ds_name) if ds_name else self._get_default_driver()
        self._mark_dirty(drv)
    ...
    def begin(self, name: str = ""):
        """开启事务/命名 savepoint——默认数据源立即开点并压栈；
        之后首次变脏的驱动会在其连接上按栈序懒开同名 savepoint。

        跨库事务无法原子协调（SQLite 跨库本来就不能一个事务，
        两阶段提交超出本层范围），跨库写的一致性由"每库一个同名
        savepoint 域"提供（回滚按库各自回滚到本库的点）；
        跨库操作请使用幂等设计 + 补偿事务（saga）。
        """
        if name:
            self._savepoint_stack.append(name)
            # 已在脏集中的驱动（savepoint 前已有写）也补开——其后的写纳入本点域
            for drv in self._dirty_drivers:
                if drv is not self._get_default_driver():
                    drv.begin(name)
    ...
    def commit(self):
        ...
        按成功逐个清账：某驱动提交失败时它留在脏集——
        后续 rollback/重试仍够得到它（旧实现先清账再提交，失败者直接失联）。
        """
        ...
        for drv in drivers:
            try:
                drv.commit()
                self._dirty_drivers.discard(drv)
            except Exception as e:
                if first_err is None:
                    first_err = e
        if first_err is not None:
            raise first_err  # savepoint 栈保留——调用方可 rollback(name) 收拾残局
        self._savepoint_stack.clear()

    def rollback(self, name: str = ""):
        """回滚事务——带 name 时每个脏驱动回滚到"自己的"同名 savepoint；
        不带 name 时对默认数据源 + 所有脏驱动逐一回滚

        与 commit() 同理：跨库回滚不保证原子，只保证每个涉及过的库
        都被回滚到本库的点。SAVEPOINT 是连接级概念，各库各自持有同名点。
        """
        if name:
            drivers = self._collect_tx_drivers()
            first_err = None
            for drv in drivers:
                try:
                    drv.rollback(name)
                    self._dirty_drivers.discard(drv)  # 按成功逐个清账
                except Exception as e:
                    if first_err is None:
                        first_err = e
            if first_err is not None:
                raise first_err  # 栈保留——部分驱动回滚失败时域名不可遗忘
            # 全部成功才弹栈到该 savepoint（其后入栈者一并释放）
            while self._savepoint_stack and self._savepoint_stack[-1] != name:
