...
def validate_where(where: str) -> bool:
    ...
    if re.search(r'\b(SELECT|UNION|INSERT|UPDATE|DELETE|DROP|ATTACH|PRAGMA)\b',
                 where, re.IGNORECASE):
        return False
...
def validate_type_change(old_type: str, new_type: str) -> str:
    ot = old_type.upper().split("(")[0].strip()
    nt = new_type.upper().split("(")[0].strip()
    int_t = {"INTEGER","INT","BIGINT","SMALLINT","TINYINT","SERIAL"}
    flt_t = {"FLOAT","REAL","DOUBLE","NUMERIC","DECIMAL"}
    txt_t = {"TEXT","VARCHAR","CHAR","CLOB"}
    def family(t):
        if t in int_t: return "int"
        if t in flt_t: return "float"
        if t in txt_t: return "text"
        return "other"
    f1, f2 = family(ot), family(nt)
    if f1 == f2: return ""
    if f1 == "int" and f2 == "float": return ""
    if f1 == "float" and f2 == "int":
        return f"将 {old_type} 改为 {new_type} 可能导致小数部分丢失，确认请使用 force=True"
    if f1 == "text" and f2 in ("int", "float"):
        return f"将 {old_type} 改为 {new_type} 可能导致非数字文本丢失，确认请使用 force=True"
    if f1 in ("int","float") and f2 == "text":
        return f"将 {old_type} 改为 {new_type} 数据不会丢失，但上游查询可能需要调整，确认请使用 force=True"
    return f"将 {old_type} 改为 {new_type} 类型差异较大，确认请使用 force=True"
