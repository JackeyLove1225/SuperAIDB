...
- （原 _translate_error 透传空壳已删，错误翻译统一契约层 ErrorTranslator，P2-5）

T1.1 安全加固：所有 f-string SQL 中的标识符插值统一使用
core.contract.security_contract 中的 safe_table_sql/safe_column_sql/
...
from core.contract.security_contract import (
    ...
    safe_pragma_arg, safe_savepoint_name, SecurityContract,
)
from .base import Driver


# 类型串白名单：纯字母类型名，可带 (n) 或 (m,n) 精度，如 VARCHAR(255)、DECIMAL(20,4)
# T1.2 纵深防御：上层 schema_manager._normalize_type 已有白名单兜底，
...
class SqliteDriver(Driver):
    ...
    def _safe_where(self, where: str) -> bool:
        if not where:
            return False
        try:
            from core.drivers.checks import validate_where
    ...
    def insert(self, table: str, rows: list[dict], overwrite: bool = False) -> dict:
        ...
        # T1.1: 入口校验 table（防御性，确保 f-string SQL 安全）
        SecurityContract.validate_identifier(table, "表名")
        # 唯一业务键冲突检测（通用机制，字段名来自行业 YAML 声明，P1-2）
    ...
    def update(self, table: str, set_clause: str, where: str = "") -> dict:
        ...
        # 单双引号+doubling 全感知）——权限判定与执行同源（评审四轮 S-2）
        from core.contract.security_contract import split_set_pairs, decode_sql_literal
        ...
        for field, raw in pairs:
            ...
            # T1.1: 校验 SET 子句中的字段名
            SecurityContract.validate_identifier(field, "字段名")
    ...
    def delete_by_pk(self, table: str, pk_column: str, pk_value) -> dict:
        """按主键删除——参数化绑定，调用方（前端/工具）不再手拼 WHERE（P1-10）"""
        SecurityContract.validate_identifier(table, "表名")
    ...
    def add_column(self, table: str, column: str, col_type: str, precision=None, not_null=False) -> dict:
        # T1.2: 驱动层独立校验类型串（纵深防御，不依赖上层白名单兜底）
        ...
        try:
            ...
            self.conn.commit()
            return {"ok": True, "message": f"已加入新字段：{column} ({col_type})"}
    ...
    def _build_create_sql(self, table: str, table_config: dict) -> str:
            ...
            if _fk.get("references") and _fk.get("ref_columns"):
                try:
                    ...
                    for _rc in _fk.get("ref_columns", []):
                        SecurityContract.validate_identifier(_rc, "外键引用字段")
                        for _r in _ri:
        ...
        for col in table_config.get("columns", []):
            ...
            # T1.1: 校验字段名
            SecurityContract.validate_identifier(col["name"], "字段名")
            ...
            if col.get("check"):
                from core.graph.check_templates import translate_for_dialect
        ...
        for cu in table_config.get("compound_uniques", []):
            for c in cu:
                SecurityContract.validate_identifier(c, "复合唯一字段")
        ...
        for fk in table_config.get("foreign_keys", []):
            ...
            if ref_table and ref_fk_cols:
                try:
                    ...
                    for fkc in fk.get("columns", []):
                        SecurityContract.validate_identifier(fkc, "外键字段")
                        for ri in ref_info:
            ...
            for c in fk.get("columns", []):
                SecurityContract.validate_identifier(c, "外键字段")
            for c in fk.get("ref_columns", ["id"]):
                SecurityContract.validate_identifier(c, "外键引用字段")
    ...
    def create_index(self, table: str, columns: str, unique: bool = False) -> str:
        # T1.1: 校验 table 和 columns（columns 是逗号分隔的字段名列表）
        SecurityContract.validate_identifier(table, "表名")
        col_list = [c.strip() for c in columns.split(",") if c.strip()]
        for c in col_list:
            SecurityContract.validate_identifier(c, "索引字段")
        ...
        self.conn.commit()
        return {"ok": True, "message": f"已创建索引 {name}"}
    ...
    def add_foreign_key(self, table: str, column: str, ref_table: str, ref_column: str = "id") -> dict:
        ...
        ref_type = 'INTEGER'
        # T1.1: 校验引用表名和字段名
        SecurityContract.validate_identifier(ref_table, "外键引用表")
        SecurityContract.validate_identifier(ref_column, "外键引用字段")
    ...
    def recreate_table(self, table_config: dict) -> dict:
        ...
        try:
            ...
            full_yaml = load_table_schema(table)
            if full_yaml:
            ...
            if common:
                for c in common:
                    SecurityContract.validate_identifier(c, "重建表共有字段")
            ...
            for idx in table_config.get("indexes", []):
                ...
                for c in idx.get("columns", []):
                    SecurityContract.validate_identifier(c, "重建表索引字段")
                SecurityContract.validate_identifier(idx["name"], "重建表索引名")
