...
const OFFICE_EXTS = ["pdf", "docx", "doc", "rtf", "odt", "xlsx", "xls", "ods", "pptx", "ppt", "odp"];
...
function PreviewBody() {
  ...
  const [urls, setUrls] = useState<{ raw: string; preview: string; download: string } | null>(null);
  ...
  if (error) {
    ...
    content = (
      <div className="flex h-full items-center justify-center overflow-auto p-6">
        ...
        <img src={urls.raw} alt={path} className="max-h-full max-w-full object-contain" />
      </div>
    );
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-[#ececec] px-4 py-2.5">
        <div className="min-w-0">
          <Link href="/dashboard/schema-designer" className="mr-3 text-sm text-zinc-400 hover:text-zinc-700">
            ← 返回
          </Link>
          <span className="truncate font-mono text-sm text-zinc-700" title={path}>
            {path}
          </span>
        </div>
        {urls ? (
