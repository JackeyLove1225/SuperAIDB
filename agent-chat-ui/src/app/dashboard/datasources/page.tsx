...
export default function DatasourcesPage() {
  ...
  const [tableDetails, setTableDetails] = useState<Record<string, { name: string; rows: number }[]>>({});
  const [tableErrors, setTableErrors] = useState<Record<string, string>>({});

  const fetchData = useCallback(async () => {
    try {
      const data = await apiFetch<{ datasources?: Datasource[] }>("/api/datasources", {
        timeoutMs: 10000,
      });
  ...
  const testConnection = async (name: string) => {
    ...
    try {
      ...
      setTestResults((prev) => ({
        ...prev,
        [name]: { ok: false, message: e instanceof Error ? e.message : "请求失败" },
      }));
    } finally {
      setTesting(null);
    }
  ...
  const loadTables = async (name: string) => {
    try {
      const data = await apiFetch<{ tables?: { name: string; rows: number }[] }>(
      ...
      // 加载失败置错误态（不伪装成空表），由 UI 显示错误+重试（P1-10）
      setTableErrors((prev) => ({
        ...prev,
    ...
    <div className="min-h-screen bg-white p-6 dark:bg-zinc-950">
      <div className="mx-auto max-w-5xl">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="mt-1 font-mono text-sm text-zinc-500 dark:text-zinc-400">
                      {ds.type === "sqlite" ? `📁 ${ds.database}` : `🌐 ${ds.host}/${ds.database}`}
                    </p>
                    <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
        ...
        <div className="mt-6 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-700 dark:border-blue-800 dark:bg-blue-950 dark:text-zinc-400">
          ...
          <p className="mt-1">
            数据源配置文件位于 <code className="rounded bg-blue-100 px-1 dark:bg-blue-900">config/datasources.yml</code>
          </p>
          <p className="mt-1">
            修改后点击"重新加载"按钮热重载，无需重启后端
          </p>
          <p className="mt-1">
            在表的 Schema YAML 中添加 <code className="rounded bg-blue-100 px-1 dark:bg-blue-900">datasource: 数据源名</code> 字段可指定表所属数据源
          </p>
        </div>
      </div>
    </div>
