...
export default function TableDataPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleteError(null);
    try {
      // 参数化删除：主键列/值由后端绑定，前端不再手拼 SQL WHERE
  ...
  const handleExport = async (format: "csv" | "excel" = "csv") => {
    ...
    try {
      ...
      const filename = String(json.path || "")
        .split(/[\\/]/)
        .pop();
        ...
        <div className="rounded-[16px] border border-[#ececec] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] dark:border-zinc-800 dark:bg-zinc-900">
          <div className="border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
            <span className="text-sm text-zinc-600 dark:text-zinc-400">
              第 {data.page} / {data.total_pages || 1} 页 · 每页{" "}
              {data.page_size} 条
            </span>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-800">
                    ...
                    <th className="sticky right-0 z-10 bg-zinc-50 px-3 py-2 text-left font-mono text-xs font-semibold text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">
                      操作
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {data.rows.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="sticky right-0 z-10 flex items-center gap-1 bg-white px-3 py-2 whitespace-nowrap dark:bg-zinc-900">
                        <button
          ...
          >
            ...
            <p className="mb-4 text-sm text-zinc-600 dark:text-zinc-400">
              ...
              <span className="font-mono font-medium text-zinc-900 dark:text-zinc-100">
                {pkColumn
                  ? `${pkColumn.name}=${String(deleteTarget[pkColumn.name] ?? "")}`
                  : ""}
              </span>{" "}
              这行数据吗？此操作不可撤销。
            </p>
