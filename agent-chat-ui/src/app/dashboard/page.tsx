  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const json = await apiFetch<DashboardData>(
        "/api/dashboard",  // apiFetch 自带 MGMT_API 前缀——传全址会双拼 404（五轮 P1 实测）
        {
      ...
      <div className="mx-auto max-w-7xl space-y-4 p-6">
        ...
        <div className="rounded-[16px] border border-[#ececec] bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.03)] dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-6">
            <Metric label="PID" value={s.pid?.toString() || "-"} />
            <Metric label="运行时间" value={s.uptime_human || "-"} />
            <Metric label="内存占用" value={`${s.memory_mb?.toFixed(1) || 0} MB`} />
            <Metric label="CPU" value={`${s.cpu_percent?.toFixed(1) || 0}%`} />
            <Metric
              label="MCP 能力面"
          ...
          <div className="rounded-[16px] border border-[#ececec] bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.03)] dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                />
                <Bar dataKey="行数" fill="#18181b" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg bg-zinc-50 px-3 py-2.5 dark:bg-zinc-800">
      ...
      <div className="mt-0.5 text-lg font-semibold tracking-tight tabular-nums text-zinc-900 dark:text-zinc-100">
        {value}
      </div>
    </div>
  );
