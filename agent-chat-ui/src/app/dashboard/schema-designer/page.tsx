  ...
  return [
    ...
    },
    {
      ...
      lines: r.outgoing_fks.length
        ? r.outgoing_fks.map(
            (fk) => `${fk.column} → ${fk.references}.${fk.ref_column}`,
          )
        : ["无"],
    },
    {
    ...
    <div className="flex h-[calc(100vh-49px)] flex-col bg-white dark:bg-zinc-950">
        ...
        <div className="flex items-center gap-4 text-xs text-zinc-500">
          ...
          {stats && (
            <span className="tabular-nums">
              {stats.table_count} 表 / {stats.column_count} 字段 /{" "}
              {stats.relationship_count} 关系
            </span>
          )}
          <span
            className={`flex items-center gap-1 ${graphEnabled ? "text-green-600" : "text-zinc-400"}`}
          >
            <span
              className={`h-2 w-2 rounded-full ${graphEnabled ? "bg-green-500" : "bg-zinc-400"}`}
            />
            图库{" "}
            {graphEnabled ? "已连接" : graphPending ? "启动中" : "降级模式"}
          </span>
        </div>
      </div>
        ...
        {nodes.length === 0 && (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <div className="text-lg font-medium tracking-tight text-zinc-400">
                画布为空
              </div>
              <div className="mt-1 text-sm text-zinc-400">
                点击「新建表」手动创建；想让 AI 帮你设计？在 MCP 客户端（Kimi
                Code / Claude Code 等）里说「建成数据库」
              </div>
            </div>
          </div>
        )}
      ...
      {tableDelete && (
        <ImpactConfirmDialog
          ...
          sections={
            tableDelete.report ? tableDeleteSections(tableDelete.report) : []
          }
      ...
      {edgeDelete && (
        <ImpactConfirmDialog
          ...
          sections={
            edgeDelete.report ? edgeDeleteSections(edgeDelete.report) : []
          }
