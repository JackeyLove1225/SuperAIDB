...
test("回显: 壳不产生语义（仅挂 tables/columns 子节点=向上继承，与引擎一致）", () => {
  // 防的回归：清空最后一个勾选时 deny 键被摘除，剩 tables 壳被误当全禁、
  // 全部复选框回勾（"取消到最后一个反而全勾"事故）
  assert.deepStrictEqual(scopeDenied({}), new Set());
  assert.deepStrictEqual(
    scopeDenied({ tables: { t1: { columns: { c1: { deny: ["query"] } } } } }),
    new Set(),
  );
});

test("回显: 显式 custom 裸壳 = fail-closed 全禁（引擎语义保留）", () => {
  assert.deepStrictEqual(scopeDenied({ mode: "custom" }), new Set(ALL));
});
...
test("库级勾选: 勾 delete → custom 黑名单（矩阵 ds_rules 形状）", () => {
  ...
  assert.deepStrictEqual(r, {
    datasources: { legacy: { mode: "custom", deny: ["delete"] } },
  });
});
...
test("库级勾选: 取消但有 tables → 保留壳（壳向上继承，不吞子规则）", () => {
  ...
  assert.deepStrictEqual(r, {
    datasources: {
      legacy: { tables: { t1: { mode: "custom", deny: ["delete"] } } },
    },
  });
});
...
test("库级勾选: 旧 read_only 重勾 → 旧 mode 被清（防扩散合并带回历史键）", () => {
  ...
  assert.deepStrictEqual(r1, {
    datasources: { legacy: { mode: "custom", deny: ["drop"] } },
  });
});
...
test("表级勾选: 写入 tables 节点，库级不带 mode/deny（矩阵 tbl_rules 形状）", () => {
  ...
  assert.ok(
    !("mode" in r.datasources!.primary) && !("deny" in r.datasources!.primary),
  );
  assert.deepStrictEqual(r, {
    datasources: {
      primary: {
        tables: { quota_items: { mode: "custom", deny: ["delete"] } },
      },
    },
  });
});
...
test("表级勾选: 取消且有 columns → 保留壳；无 columns → 表节点删除", () => {
  ...
  assert.deepStrictEqual(r1, {
    datasources: {
      primary: { tables: { t1: { columns: { secret: { deny: ["query"] } } } } },
    },
  });
  const r2 = withTableDenied(
    withTableDenied({}, "primary", "t1", new Set(["delete"])),
    "primary",
    "t1",
    new Set(),
  );
...
test("列级勾选: 叠加第二操作 → 同列 deny 累加；取消到最后一个 → 列节点删除", () => {
  ...
  assert.deepStrictEqual(
    r.datasources!.primary.tables!.t1.columns!.secret.deny,
    ["query", "update"],
  );
  r = withColDeny(r, "primary", "t1", "secret", "query", false);
  r = withColDeny(r, "primary", "t1", "secret", "update", false);
  assert.deepStrictEqual(r.datasources!.primary.tables!.t1.columns, {}); // 空壳留待 pruneShells 摘
});
...
test("摘壳: 空 deny 列、空 tables 全部摘除；有效规则原样通过", () => {
  const r: RulesDoc = {
    default: "full",
    datasources: {
      primary: {
  ...
  assert.deepStrictEqual(pruneShells(r), {
    default: "full",
    datasources: {
      primary: { tables: { t2: { mode: "custom", deny: ["ddl"] } } },
    },
  });
});
...
test("端到端: UI 操作链 → prune → 与矩阵 col_rules(['query']) 形状逐字节一致", () => {
  ...
  assert.deepStrictEqual(pruneShells(r), {
    ...
    datasources: {
      primary: {
        tables: { t1: { columns: { secret: { deny: ["query", "update"] } } } },
      },
    },
  });
});
...
// ── users.<用户名> 路径：与 datasources 同构但根键不同（表不挂数据源下）──

test("用户操作级勾选: 勾 delete → users.<u> custom 黑名单；清空 → 用户节点删除", () => {
  let r = withUserDenied({}, "user_1", new Set(["delete"]));
  assert.deepStrictEqual(r, {
    users: { user_1: { mode: "custom", deny: ["delete"] } },
  });
  r = withUserDenied(r, "user_1", new Set());
  assert.deepStrictEqual(r, { users: {} });
});

test("用户表级勾选: 写入 users.<u>.tables，用户节点不被污染；列级回读一致", () => {
  let r = withUserTableDenied({}, "user_1", "quota_items", new Set(["query"]));
  assert.ok(
    !("mode" in r.users!.user_1) && !("deny" in r.users!.user_1),
    "表级勾选不得污染用户操作级节点",
  );
  assert.deepStrictEqual(r, {
    users: {
      user_1: {
        tables: { quota_items: { mode: "custom", deny: ["query"] } },
      },
    },
  });
  r = withUserColDeny(r, "user_1", "quota_items", "base_price", "query", true);
  assert.deepStrictEqual(
    r.users!.user_1.tables!.quota_items.columns!.base_price.deny,
    ["query"],
  );
  assert.equal(
    userColDenied(r, "user_1", "quota_items", "base_price", "query"),
    true,
  );
  assert.equal(
    userColDenied(r, "user_1", "quota_items", "base_price", "update"),
    false,
  );
});

test("用户规则: 清空操作级但有 tables → 保留壳；pruneShells 同样摘 users 空壳", () => {
  let r = withUserDenied({}, "user_1", new Set(["delete"]));
  r = withUserTableDenied(r, "user_1", "t1", new Set(["query"]));
  r = withUserDenied(r, "user_1", new Set());
  assert.deepStrictEqual(r, {
    users: {
      user_1: { tables: { t1: { mode: "custom", deny: ["query"] } } },
    },
  });
  // 空壳（无任何语义键/子节点）被摘除；有效 users 规则原样通过
  //（pruneShells 既有行为：datasources 键即使原本缺失也会补空对象，与 roles:{} 同理）
  const dirty: RulesDoc = {
    users: {
      ghost: {},
      user_1: { tables: { t1: { mode: "custom", deny: ["query"] } } },
    },
  };
  assert.deepStrictEqual(pruneShells(dirty), {
    datasources: {},
    users: { user_1: { tables: { t1: { mode: "custom", deny: ["query"] } } } },
  });
});
