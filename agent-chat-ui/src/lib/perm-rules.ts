...
export const OPS = ["query", "insert", "update", "delete", "ddl", "drop"] as const;
...
export interface RulesDoc {
  ...
  roles?: Record<string, { allow?: Op[]; deny?: Op[] }>;
}

// 规则结构 → 禁止集合（复选框显示语义）：
// 无覆盖/full=全放；read_only=禁查询外全部；custom+deny=黑名单位；
// custom+allow=白名单补集；custom 裸壳=fail-closed 全禁。
export function scopeDenied(scope?: ScopeRules): Set<Op> {
  if (!scope) return new Set();
  if (scope.mode === "full") return new Set();
  if (scope.mode === "read_only") return new Set(OPS.filter((o) => o !== "query"));
  if (scope.allow !== undefined) return new Set(OPS.filter((o) => !scope.allow!.includes(o)));
...
export function pruneShells(rules: RulesDoc): RulesDoc {
  ...
  const dsMap: NonNullable<RulesDoc["datasources"]> = {};
  for (const [ds, scope] of Object.entries(out.datasources || {})) {
      ...
      for (const [t, ts] of Object.entries(s.tables)) {
        ...
        if (t2.columns) {
          const cMap = Object.fromEntries(
            Object.entries(t2.columns).filter(([, c]) => (c.deny || []).length > 0)
          );
          if (Object.keys(cMap).length) t2.columns = cMap; else delete t2.columns;
        }
        if (hasScope(t2) || t2.columns) tMap[t] = t2;
      }
      if (Object.keys(tMap).length) s.tables = tMap; else delete s.tables;
    }
    if (hasScope(s) || s.tables) dsMap[ds] = s;
  }
  out.datasources = dsMap;
  if (out.roles) {
    out.roles = Object.fromEntries(
      Object.entries(out.roles).filter(([, r]) => r.allow !== undefined || r.deny !== undefined)
    );
  }
  return out;
}
...
export function withDsDenied(r: RulesDoc, ds: string, denied: Set<Op>): RulesDoc {
  const dsMap = { ...(r.datasources || {}) };
  const prev = { ...(dsMap[ds] || {}) };
  delete prev.mode; delete prev.allow; delete prev.deny;
  if (denied.size === 0) {
    if (!prev.tables) delete dsMap[ds]; else dsMap[ds] = prev;
  } else {
...
export function withTableDenied(r: RulesDoc, ds: string, table: string, denied: Set<Op>): RulesDoc {
  const dsMap = { ...(r.datasources || {}) };
  ...
  delete prev.mode; delete prev.allow; delete prev.deny;
  if (denied.size === 0) {
    if (!prev.columns) delete tables[table]; else tables[table] = prev;
  } else {
...
export function withColDeny(
  r: RulesDoc, ds: string, table: string, col: string, op: Op, denied: boolean
): RulesDoc {
  const dsMap = { ...(r.datasources || {}) };
  const dsScope = { ...(dsMap[ds] || {}) };
  const tables = { ...(dsScope.tables || {}) };
  ...
  if (denied) denySet.add(op); else denySet.delete(op);
...
export function colDenied(rules: RulesDoc, ds: string, table: string, col: string, op: Op): boolean {
  const c = rules.datasources?.[ds]?.tables?.[table]?.columns?.[col];
  return !!c?.deny?.includes(op);
}

export function withRoleRules(
  r: RulesDoc, name: string, patch: Partial<{ allow: Op[]; deny: Op[] }>
): RulesDoc {
  const rolesMap = { ...(r.roles || {}) };
  const cur = { ...(rolesMap[name] || {}) };
  delete cur.allow; delete cur.deny;
  Object.assign(cur, patch);
  if (!cur.allow && !cur.deny) delete rolesMap[name];
  else rolesMap[name] = cur;
  return { ...r, roles: rolesMap };
}
