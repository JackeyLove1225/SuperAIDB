...
test("非 2xx: detail 字符串原样成为错误消息", async () => {
  ...
  await assert.rejects(
    apiFetch("/api/x"),
    (e) =>
      e instanceof ApiError && e.status === 404 && e.message === "表不存在",
  );
});
...
test("非 2xx: FastAPI 422 detail 数组整形成可读文本（去 body/query 前缀，字段路径点连）", async () => {
  ...
  await assert.rejects(
    ...
    (e) =>
      ...
      e.status === 422 &&
      e.message ===
        "rows.0.name: field required; limit: value is not a valid integer",
  );
});
...
test("5xx 重试: retries=1 时首次 500、第二次成功则返回成功结果", async () => {
  ...
  const r = await apiFetch<{ value: number }>("/api/x", {
    retries: 1,
    backoffBaseMs: 1,
  });
...
test("5xx 重试: 重试额度耗尽后抛最后一次的 ApiError", async () => {
  ...
  await assert.rejects(
    ...
    (e) =>
      e instanceof ApiError && e.status === 503 && e.message === "持续故障",
  );
  assert.equal(calls.length, 2);
});
