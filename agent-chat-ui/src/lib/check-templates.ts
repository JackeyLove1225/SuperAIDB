...
export async function loadCheckTemplates(
  colType: string,
  ...
  try {
    ...
    const templates: CheckTemplate[] = (data.templates || []).map(
      parseTemplate,
    );
...
export function renderParam(
  ...
  if (kind === "int_list" || kind === "float_list") {
    return toList(value)
      .map((v) =>
        kind === "int_list" ? parseInt(String(v)) : parseFloat(String(v)),
      )
      .filter((n) => !isNaN(n))
      .join(", ");
  }
