...
type Snap = { values: Record<string, FieldValue>; nulls: Record<string, boolean> };
const snap = (values: Snap["values"], nulls: Snap["nulls"] = {}): Snap => ({ values, nulls });

// 组件不变式：values 在初始化时为每列都建档（缺省 ""），测试按同一契约构造完整值表
const vals = (over: Record<string, FieldValue>): Record<string, FieldValue> => ({
  uid: "", name: "", age: "", active: "", note: "",
  ...over,
});
...
test("新增: 布尔列 1/'1'→1，其余→0", () => {
  assert.deepStrictEqual(buildRowValues(vals({ name: "x", active: "1" }), {}, COLS), { name: "x", active: 1 });
  assert.deepStrictEqual(buildRowValues(vals({ name: "x", active: 1 }), {}, COLS), { name: "x", active: 1 });
  assert.deepStrictEqual(buildRowValues(vals({ name: "x", active: "0" }), {}, COLS), { name: "x", active: 0 });
});
...
test("编辑: 值变更 → dirty；未修改提交 → 空集", () => {
  const orig = snap({ uid: "1", name: "张三", age: "30", active: "1", note: "旧" });
...
test("编辑: 空串是合法值——文本列清空为 '' 是 dirty，须原样提交（keepEmpty）", () => {
  const orig = snap({ uid: "1", name: "张三", age: "30", active: "1", note: "旧备注" });
...
test("编辑: NULL 勾选双向——勾选（false→true）dirty；保持勾选且未动 不 dirty", () => {
  const orig = snap({ uid: "1", name: "x", age: "30", active: "1", note: "旧" });
