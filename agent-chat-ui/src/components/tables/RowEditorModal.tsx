...
export default function RowEditorModal({
  open, mode, tableName, columns, rowData, onClose, onSuccess,
}: RowEditorModalProps) {
  ...
  const origRef = useRef<{ values: Record<string, FieldValue>; nulls: Record<string, boolean> }>({
    values: {},
    nulls: {},
  });
    ...
    try {
      if (mode === "create") {
        ...
        const json = await apiFetch<{ ok?: boolean; message?: string }>(
          ...
          { method: "POST", body: JSON.stringify({ rows: [row], overwrite: false }) }
        );
        if (json && json.ok === false) throw new Error(json.message || "插入失败");
      } else {
        // 编辑 = 按主键参数化更新（update-by-pk）：只提交 dirty 字段（空串是合法值）。
        // 类型整形复用 buildRowValues，SET/WHERE 由服务端按 JSON 类型安全拼装——前端不再手拼 SQL（P1-10 同哲学）
        ...
        if (json && json.ok === false) throw new Error(json.message || "更新失败");
      }
      onSuccess();
      onClose();
    } catch (e) {
    ...
    <DraggableModal
      title={
        <span>
          ...
          <span className="ml-2 font-mono text-sm text-zinc-400">{tableName}</span>
        </span>
      }
      onClose={onClose}
      initialWidth={700}
    >
      ...
      {mode === "edit" && pkColumn && (
        <div className="mb-4 rounded-md bg-zinc-50 px-4 py-2 dark:bg-zinc-800">
          <span className="font-mono text-xs text-zinc-400">主键 {pkColumn.name}（只读）</span>
      ...
      <div className="space-y-3">
        {editableColumns.map((col) => {
          ...
          return (
            <div key={col.name} className="flex items-start gap-3 border-b border-zinc-100 pb-3 last:border-0 dark:border-zinc-800">
              <div className="w-40 shrink-0 pt-2">
                <div className="font-mono text-sm font-medium text-zinc-700 dark:text-zinc-300">{col.name}</div>
                <div className="text-xs text-zinc-400">{col.type}</div>
                {col.not_null && <span className="text-xs text-red-400">NOT NULL</span>}
              </div>
