...
export function AppSidebar() {
  ...
  const bottomLinks = [
    ...
    },
    {
      ...
      label: "表设计",
      icon: Table2,
      active: pathname.startsWith("/dashboard/schema-designer"),
    },
    {
      href: "/dashboard/datasources",
      label: "数据源",
      icon: Database,
      active: pathname === "/dashboard/datasources",
    },
    {
      href: "/dashboard/permissions",
      label: "权限",
      icon: ShieldCheck,
      active: pathname === "/dashboard/permissions",
    },
    {
      href: "/settings",
      label: "设置",
  ...
  // 当前登录用户（20260903 改用 useAuth 订阅 auth store 单一事实来源：
  // 根 layout 常驻的侧边栏在 SPA 客户端跳转（登录页 → /）后不重新挂载，
  // 此前的本地 state + 一次性 fetchMe 导致"退出后重登看不到退出登录按钮"）
  const { me } = useAuth();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-5 pt-4 pb-3">
        ...
        <span className="text-[15px] font-medium tracking-tight text-[#0d0d0d]">
          SuperAIDB
        </span>
      </div>
      ...
      <div className="mt-auto border-t border-[#ececec] p-2">
        ...
        {!isSystemMode && me && (
          <div className="mt-1 flex items-center gap-2 rounded-lg border-t border-[#ececec] px-3 pt-2 pb-1">
            ...
            <span
              className="min-w-0 flex-1 truncate text-[13px] text-[#0d0d0d]"
              title={me.username}
            >
              {me.username}
            ...
            <button
              ...
              onClick={logout}
              title="退出登录"
              className="flex items-center gap-1 text-[12px] text-[#9e9e9e] transition-colors hover:text-[#0d0d0d]"
            >
              <LogOut
                className="size-4"
                strokeWidth={1.75}
