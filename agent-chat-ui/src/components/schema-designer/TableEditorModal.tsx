...
export default function TableEditorModal({
  ...
  const defaultDs = presetDatasource
    ...
    initialData?.columns && initialData.columns.length > 0
      ? // 深拷贝：避免修改字段时 mutate 画布节点的原始对象（问题1根因）
        ...
        initialData.columns.map((c) => ({
          ...c,
          check_template_params: c.check_template_params ? { ...c.check_template_params } : undefined,
        }))
      : [
          { name: "id", type: "INTEGER", is_pk: true, not_null: true, autoincrement: true, description: "主键，自增" },
        ]
  );
  const [saving, setSaving] = useState(false);
  const [pendingRiskReport, setPendingRiskReport] = useState<RiskReport | null>(null);
  ...
  useEffect(() => {
    if (mode === "edit" && initialData?.columns) {
      ...
      initialData.columns.forEach((c, i) => { map[i] = c.type; });
  ...
  const addColumn = () => {
    setColumns([
      ...columns,
      { name: "", type: "TEXT", not_null: false, is_unique: false, is_indexed: false, check_constraint: "", description: "" },
    ]);
  };
  ...
  const updateColumn = <K extends keyof ColumnData>(idx: number, field: K, value: ColumnData[K]) => {
  ...
  const handleSave = async () => {
    ...
    if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(name)) {
      toast.error("表名必须以字母或下划线开头，只含字母、数字、下划线");
      return;
    }
    // 确保有 id 主键（需求4：id 默认主键，无需复选框）
    const hasId = columns.some((c) => c.name === "id");
    if (!hasId) {
      toast.error("每张表必须包含 id 主键字段");
      return;
    }
    // 校验字段名
    for (const col of columns) {
      ...
      if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(col.name)) {
        toast.error(`字段名 '${col.name}' 不合法，必须以字母或下划线开头`);
        return;
      }
    }
    ...
    for (const col of columns) {
      ...
      try {
        ...
        if (!data.ok) {
          toast.error(`字段 '${col.name}' 的 CHECK 约束非法:\n${data.message}\n\n表达式: ${col.check_constraint}`);
          return;
        }
      } catch (e) {
        toast.error(`字段 '${col.name}' 的 CHECK 校验请求失败: ${e instanceof ApiError ? e.message : String(e)}`);
        return;
      }
    }
    ...
    if (mode === "edit") {
      try {
        const precheckData = await apiFetch<{ ok?: boolean; report?: RiskReport; detail?: string }>(
          `/api/schema-graph/table/${name}/precheck`,
          {
            method: "POST",
            body: JSON.stringify({
              updates: { business_name: businessName, description, datasource, columns: finalColumns },
            }),
          }
        );
        ...
        if (e instanceof ApiError) {
          ...
          toast.error(`预校验失败: ${e.message}`);
          return;
        }
        // precheck 失败 fail-closed（P1-10）：风险预校验不可用时中止保存，
        // 不让潜在高危操作绕过安全确认
        toast.error(`保存前风险预校验失败，已中止保存: ${e instanceof Error ? e.message : e}`);
        return;
      }
    }

    // 正常保存（无风险 / 新建模式 / precheck 降级）
    await doSave(finalColumns, false);
  };

  /** 实际执行保存请求 */
  const doSave = async (finalColumns: ColumnData[], force: boolean) => {
    setSaving(true);
    const schema = {
      name,
    ...
    try {
      ...
      const body =
        mode === "create"
          ? { table_schema: schema, x: 100, y: 300, create_real_table: true }
          : { updates: { business_name: businessName, description, datasource, columns: finalColumns }, force };

      await apiFetch(path, { method, body: JSON.stringify(body) });

      // 问题4：保存成功后，更新 committed types 为本次保存的类型（作为下次风险拒绝的回滚基准）
      finalColumns.forEach((c, i) => { committedTypesRef.current[i] = c.type; });
      onSaved();
    } catch (e) {
      // 处理 409 风险确认响应（响应体经 ApiError.data 取回）
      if (e instanceof ApiError && e.status === 409) {
        const data = (e.data ?? {}) as { report?: RiskReport; message?: string };
        if (data.report) {
          setPendingRiskReport(data.report);
          return;
        }
        toast.error(`保存失败: ${data.message || "变更包含高危操作，需确认"}`);
        return;
      }
      toast.error(`保存失败: ${e instanceof ApiError ? e.message : String(e)}`);
    } finally {
      setSaving(false);
    }
  };

  /** 用户确认风险后，带 force=true 重新保存 */
  const confirmAndForceSave = () => {
    const finalColumns = columns.map((c) =>
      c.name === "id"
        ? { ...c, is_pk: true, not_null: true, autoincrement: true }
        : { ...c, is_pk: false }
    ...
    columns.forEach((c, i) => { committedTypesRef.current[i] = c.type; });
    setPendingRiskReport(null);
    doSave(finalColumns, true);
  };
  ...
  return (
    <ModalOverlay onClose={onClose} title={mode === "create" ? "➕ 新建表" : `✏️ 编辑表: ${name}`}>
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-amber-200 bg-amber-50/50 p-2 dark:border-amber-700 dark:bg-amber-950/20">
          ...
          {mode === "edit" && (
            <p className="mt-1 text-[10px] text-zinc-400">编辑模式下不可更改数据源（如需迁移请删除后重建）</p>
          )}
        </div>
