...
const FIELD_TYPES: { name: string; desc: string }[] = [
  {
    name: "INTEGER",
    desc: "整数类型。适用于计数、ID、状态码等不带小数的数值。如年龄、数量、外键引用 id。",
  },
  {
    name: "TEXT",
    desc: "文本类型。适用于名称、描述、地址等字符串。如用户名、商品标题、备注。",
  },
  {
    name: "REAL",
    desc: "浮点类型。适用于带小数的数值，精度要求不高。如温度、评分、比例。",
  },
  {
    name: "NUMERIC",
    desc: "数值类型。适用于需要精确计算的小数，如金额、汇率。优先用 REAL 替代以简化。",
  },
  {
    name: "DATE",
    desc: "日期类型。仅存储日期（年-月-日）。如出生日期、创建日期。存储为 TEXT 格式 'YYYY-MM-DD'。",
  },
  {
    name: "DATETIME",
    desc: "日期时间类型。存储日期+时间。如订单时间、最后登录时间。存储为 TEXT 格式 'YYYY-MM-DD HH:MM:SS'。",
  },
  {
    name: "BLOB",
    desc: "二进制大对象。适用于存储图片、文件等二进制数据。一般不建议直接存数据库，推荐存路径。",
  },
];

// ── 约束选项说明（需求5：悬停显示含义及使用场景）──
const CONSTRAINT_INFO: {
  key: "not_null" | "is_unique" | "is_indexed";
  label: string;
  desc: string;
  color: string;
}[] = [
...
export type ColumnUpdateFn = <K extends keyof ColumnData>(
  field: K,
  value: ColumnData[K],
) => void;
...
export default function ColumnEditor({
  ...
  const [showCheckEditor, setShowCheckEditor] = useState(
      ...
      {!isIdField && showCheckEditor && (
        <div className="mt-1.5 rounded border border-green-200 bg-green-50/50 p-2 dark:border-green-800 dark:bg-green-950/20">
          ...
          <div className="flex min-w-0 items-center gap-1.5">
            ...
            >
              <option value="">
                选择模板（按类型 {normalizeType(col.type)}）...
              </option>
              {templates.map((t) => (
                <option
                  key={t.key}
            ...
            checkTemplateKey !== "custom" && (
              <div className="mt-1.5 grid grid-cols-2 gap-1.5">
                {selectedTemplate.params.map((p) => (
                  ...
                  >
                    <label className="shrink-0 font-mono text-[10px] text-zinc-600 dark:text-zinc-400">
                      {p.name}:
                    </label>
                    <input
                      type={
...
function TypeSelectWithTooltip({
  ...
  const currentDesc =
    FIELD_TYPES.find((t) => t.name === value)?.desc || "未知类型";
  ...
  return (
    <div className="relative shrink-0">
      ...
      {showTip && (
        <div className="absolute top-full left-0 z-50 mt-1 w-56 rounded border border-amber-200 bg-amber-50 p-2 text-xs leading-relaxed text-amber-900 shadow-lg dark:border-amber-700 dark:bg-amber-950 dark:text-amber-200">
