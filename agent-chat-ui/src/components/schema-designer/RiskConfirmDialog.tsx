        ...
        <span className="flex items-center gap-2">
          ...
          <span>
            ...
            <span className={`ml-2 text-sm font-normal ${style.text}`}>
              风险等级：{style.label} ·{" "}
              {report.summary || `${changes.length} 项变更`}
            </span>
          </span>
        </span>
      }
              ...
              {c.data_impact && Object.keys(c.data_impact).length > 0 && (
                <div className="mt-2 rounded bg-black/5 p-2 text-xs dark:bg-white/5">
                  {c.data_impact.fail_count != null && (
                    <span className="mr-3">
                      采样 {c.data_impact.scanned || 0} 行，
                      {c.data_impact.fail_count} 行无法转换
                    </span>
                  )}
                  ...
                  {c.data_impact.fail_samples &&
                    c.data_impact.fail_samples.length > 0 && (
                      <div className="mt-1 text-zinc-500">
                        失败样本:{" "}
                        {c.data_impact.fail_samples.slice(0, 3).join(", ")}
                      </div>
                    )}
                </div>
              )}
