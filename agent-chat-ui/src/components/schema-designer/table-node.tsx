...
export interface TableNodeData {
  ...
  columns: ColumnData[];
  foreign_keys?: { columns: string[]; references: string; ref_columns: string[] }[];
    ...
    >
      ...
      <div className="flex items-center justify-between rounded-t-[11px] bg-zinc-900 px-3 py-1.5 dark:bg-zinc-800">
        <div className="min-w-0 flex-1">
          ...
          {d.business_name && (
            <div className="truncate pl-4 text-xs text-zinc-300">{d.business_name}</div>
          )}
        </div>
      ...
      {!showColumns && (
        <div className="cursor-pointer px-3 py-2 text-xs text-zinc-500 dark:text-zinc-400">
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="font-medium text-zinc-700 dark:text-zinc-300" title="主键">🔑 {allColumns.filter(c => c.is_pk).length}</span>
            {fkCount > 0 && (
              <span className="text-zinc-500 dark:text-zinc-400" title="外键字段">🔗 {fkCount}</span>
            )}
            {uniqueCount > 0 && (
              <span className="text-zinc-500 dark:text-zinc-400" title="唯一约束">U {uniqueCount}</span>
            )}
            {indexedCount > 0 && (
              <span className="text-zinc-500 dark:text-zinc-400" title="索引">I {indexedCount}</span>
            )}
            {checkCount > 0 && (
              <span className="text-zinc-500 dark:text-zinc-400" title="CHECK约束">✓ {checkCount}</span>
            )}
          </div>
          <div className="mt-1 text-[10px] text-zinc-400">点击卡片展开字段列表</div>
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                <span className="flex shrink-0 items-center gap-0.5">
                  {col.not_null && !isPK && (
                    <span className="text-zinc-400" title="非空">*</span>
                  )}
                  {col.is_unique && !isPK && (
                    <span className="text-zinc-500" title="唯一约束">U</span>
                  )}
                  {col.is_indexed && (
                    <span className="text-zinc-500" title="普通索引">I</span>
                  )}
                  {col.check_constraint && (
                    <span className="text-zinc-600" title={`CHECK: ${col.check_constraint}`}>✓</span>
                  )}
                </span>
              </div>
            );
